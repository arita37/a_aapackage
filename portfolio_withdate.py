...
from calendar import isleap;                 from dateutil import parser
...
def datetime_tostring(tt):
  if isinstance(tt, datetime.date) :    return tt.strftime("%Y%m%d")
  if isinstance(tt, np.datetime64 ) :   t= pd.to_datetime(str(tt)) ;  return t.strftime("%Y%m%d") 

  if isinstance(tt[0], np.datetime64 ) :     tt= pd.to_datetime(str(tt))  
  date2 = []
  for t in tt:
...
def datetime_todate(tt):
 ...
 if isinstance(tt, np.datetime64 ) :     tt= pd.to_datetime(str(tt)) ;  return datetime.date(tt.year, tt.month, tt.day )

 if isinstance(tt[0], np.datetime64 ) :     tt= pd.to_datetime(tt)
 date2 = []
 for t in tt:
...
def date_align(quotes, dateref=None, datestart=19550101, type1="close"):
 ...
 for k in range(0, masset):
  ...
  if not isint(df.date.values[0]) : datei=  np.array(datetime_toint(df.date.values))
