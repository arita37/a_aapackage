...
print sys.argv, "\n\n", sys.argv[0], "\n\n", sys.argv[1], "\n\n"
'''
# Get Task Folder from Arg Script Name
title1=      sys.argv[0]
...
filedataxxx, itask0, itask1  = args1[0]

#Other data
args2= args1[1]

print("Ok", title1, (itask0, itask1), args2 )
# task_folder= 'elvis_prod_28asset_1'
# title1 ='test'


################### Argument  from config ########################################################
execfile(DIRBATCH + '/ec2_config.py')
print( filedata, input_param_file, batch_in_data1, batch_script )
...
batch_script=     DIRBATCH + batch_script      #  '/elvis_pygmo_optim.py'
batch_in_data1=   DIRBATCH + batch_in_data1    #  '/aafolio_storage_ref.pkl'
filedata=         DIRBATCH + filedata          #  '/close_28assets_2012_jan2017.spydata'



# dir_batch_main=   DIRCWD + '/linux/batch/'
batch_out_name=   'batch_' + util.date_nowtime('stamp')   #str(np.random.randint(1000, 99999999))
batch_out=        DIRCWD + '/linux/batch/output/'+date0 + '/'+ batch_out_name
...
input_params_list=   util.py_load_obj(DIRBATCH + input_param_file, isabsolutpath=1)
...
http://chriswhong.github.io/nyc-heatmap/
