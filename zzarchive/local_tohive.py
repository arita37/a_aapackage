def local_tohive(folder, tablename):
    try:
        ...
        with open(hiveql, mode="w") as f:
            ...
            f.write(
                "CREATE TABLE {} LIKE {} ROW FORMAT SERDE 'org.apache.hadoop.hive.serde2.OpenCSVSerde' WITH SERDEPROPERTIES ('separatorChar' = ',') STORED AS TEXTFILE TBLPROPERTIES('skip.header.line.count' = '1');\n".format(
                    csvtable, tablename
                )
            )
            f.write(
                "LOAD DATA LOCAL INPATH '{}*.csv' OVERWRITE INTO TABLE {};\n".format(
                    foldr, csvtable
                )
            )
