...
s           "task01" :
        ...
        for fd_name in self.task_list.keys():
            for script in self.task_list[fd_name].task_script_list:
                ...
                if not self.script_header_isok(fn):
                    self.log(
                        "ERROR: script %s does not have a good header so"
                    ...
                    self.log(
                        "script %s has been processed, and the header" " has been executed" % script
                    )
    ...
    def folder_create_structure(self, name, task):
        ...
        if not os.path.isdir(name):
            self.log("%s name is not a directory, please" "put scripts inside of a directory")
            return
        ...
        for script_name in scripts:
            ...
            self.task_list[task]["task_script_list"][script_name] = AttrDict(
                {
                    "script_pid": None,
                    "script_status": None,
            ...
            txts = (
                "%s%s%s%s%s_std%s.txt" % (name, os.sep, "output", os.sep, script_name, "out"),
                "%s%s%s%s%s_std%s.txt" % (name, os.sep, "output", os.sep, script_name, "err"),
            )
            for txt in txts:
                open(txt, "w").close()
    ...
    def folder_retrieve_task(self):
        ...
        for folder in os.listdir(self.folder_root):
            if "task" in folder and os.path.isdir(self.folder_root + folder):
                ...
                self.task_list[folder] = AttrDict(
                    {"task_status": None, "task_startime": None, "task_script_list": dict()}
                )
    ...
    def task_move_failed(self, task_name):
        ...
        if self.task_list[task_name].task_status == 1 and "failed" not in task_name:
            self.log(
                "%s %s %s" % (task_name, "has failed so is beeing renamed to", renamed_folder_name)
            )
    ...
    def run_command(self, command):
        ...
        try:
            ...
            origWD = os.getcwd()
            os.chdir(cwd)
            num = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False)
            os.chdir(origWD)
            if sys.platform.startswith("win"):
    ...
    def data_split(self, NSPLIT, ar, folder_name):
        ...
        for ii in range(0, NSPLIT):
            ...
            py_save_object(
                ar_i,
                "%s%s%s%s%s%s" % (folder_name, os.sep, "input", os.sep, output_varname, str(ii)),
                1,
            )
    ...
    def task_isfinished(self, task):
        ...
        if dtask.task_status == 0:
            srcipts_states = map(
                lambda x: dtask.task_script_list[x].script_status, dtask.task_script_list
            )
    ...
    def task_all_running(self):
        states = map(lambda x: self.task_list[x].task_status, self.task_list)
        if 0 in states:
            return True
        else:
    ...
    def monitor_task(self, task):
        ...
        if not self.task_isok(task=task):
            ...
            self.log(
                "task: %s is not complete there is a problem in"
    ...
    def monitor_script(self, script, task, pid):
        ...
        stdout_fname = (
            self.folder_root + task + os.sep + "output" + os.sep + "%s_stdout.txt" % script
        )
        stderr_fname = (
            self.folder_root + task + os.sep + "output" + os.sep + "%s_stderr.txt" % script
        )
        if sys.platform.startswith("win"):
...
def execute(folder_root, startime="2010-03-01 11:25", timeout="2020-01-01 11:00"):
