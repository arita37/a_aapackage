...
########################  Task 1 on Spot Instance   ######################################

#### 1) Create Elastic IP for Spot Instance + Task Folder ################################
host=    '52.79.79.1'   #Static version

##  Create Elastic IP, Create Instance Spot Bid, Assign IP adresss
params= {
'task1_name':   'elvis_prod_20161228' ,      # Folder of  linux/batch/task/
'NCPU':         16+5                  ,      # 16 CPU: 80% Load
'EC_id':       'i-0f2567ef36bd265d5'         #Instance ID
}
globals().update(params)
host= util.aws_ec2_allocate_elastic_ip(instance_id=EC_id);  params['host']= host

DIRBATCH  =   DIRCWD + '/linux/batch/task/' + task1_name
util.os_config_setfile(params, DIRBATCH+'/ec2_config.py',  mode1='w+')
...
'''
conda install -y -c conda-forge pygmo=1.1.7
pip install kmodes
pip install ggplot

mkdir -p -v  notebook/linux/batch/task

mkdir -p -v  linux/batch/output
...
#### 3)  Modify the Batch_launcher FILE for number of CPU  ##############################
''' 1) Change batch_param_generator.py to generate the list of params
...
ssh.put_all(DIRCWD + task1_folder, EC2CWD + '/linux/batch/task' )       #  3) Copy Local  project27/aapackage  to awsprod/aapackage
"""
...
print(EC2CWD + task1_folder+'/batch_launcher_02.py')
...
execfile( DIRBATCH+'/ec2_config.py') ;     print 'Nb CPU:', NCPU, 'host:', host, task1_name
