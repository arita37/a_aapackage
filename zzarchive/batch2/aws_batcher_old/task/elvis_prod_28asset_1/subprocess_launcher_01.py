...
try :
  ...
  task_folder= fromfile1.split('/')[-2]            #  print 'Task Folder', task_folder
...
print  'Nb CPU: ', NCPU, '\n', 'Directory Batch:', DIRBATCH, '\n', 'Input Param:', input_param_file
...
# NCPU=  10
nhour= 3.0
input_param= util.py_load_obj( DIRBATCH + input_param_file  ,isabsolutpath=1)
ntask= input_param.shape[0]; del input_param

ncpu_time=  NCPU * nhour
nsplit=     NCPU
splitsize = max(1, int(np.round(ntask / NCPU, 0)))
...
# subprocess_script is defined  in config file
script_list= [ (DIRBATCH + subprocess_script, '0,0')  for i in xrange(0, nsplit) ]
...
for ii, task_id in enumerate(task_id_list) :
  ...
  print 'CPU_ID:'+str(ii), (j0,j1), filescript, str(( task_id , args1))
