...
class FeedForwardModel(object):
        ...
        with tf.variable_scope("forward"):
            for t in range(0, self._num_time_interval - 1):
                ...
                if self._usemodel == 'lstm':
                    z = self._subnetworklstm([self._x[:, :, t + 1]], str(t + 1)) / self._dim
    ...
    def _subnetworklstm(self, x, name):
        with tf.variable_scope(name):
            lstm = tf.nn.rnn_cell.LSTMCell(self._config.n_hidden_lstm)
            x, s = tf.nn.static_rnn(lstm, x, dtype=TF_DTYPE)
            x = tf.layers.dense(x[-1], self._config.num_hiddens[-1])
            return x
