...
class FeedForwardModel(object):
    ...
    def train2(self):
        ...
        dw_valid = np.reshape(dw_valid,
                              [self._config.batch_size, self._config.clayer * self._dim, self._num_time_interval])
        x_valid = np.reshape(x_valid,
                             [self._config.batch_size, self._config.clayer * self._dim, self._num_time_interval + 1])
        feed_dict_valid = {self._dw: dw_valid, self._x: x_valid, self._is_training: False}
        ...
        for step in range(self._config.num_iterations + 1):
            ...
            dw_train = np.reshape(dw_train,
                                  [self._config.batch_size, self._config.clayer * self._dim, self._num_time_interval])
            x_train = np.reshape(x_train, [self._config.batch_size, self._config.clayer * self._dim,
