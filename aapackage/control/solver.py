...
class FeedForwardModel(object):
    ...
    def train(self):
        ...
        for step in range(self._config.num_iterations+1):
            ...
            if step % self._config.logging_frequency == 0:
                ...
                log("step: %5u,    loss: %.4e,   Y0: %.4e,  elapsed time %3u" % (step, loss, init, elapsed_time))
    ...
    def build(self):
        ...
        self._y_init      = tf.Variable(tf.random_uniform([1],
                                                     ...
                                                     dtype=TF_DTYPE))
        ## Control                                             
        z_init      = tf.Variable(tf.random_uniform([1, self._dim], minval=-.1, maxval=.1,
                                                    dtype=TF_DTYPE))
        ...
        with tf.variable_scope('forward'):
            for t in range(0, self._num_time_interval-1):
                y = y - self._bsde.delta_t * (self._bsde.f_tf(time_stamp[t], self._x[:, :, t], y, z)) \
            ...
            # Terminal time
            y = y - self._bsde.delta_t * self._bsde.f_tf(time_stamp[-1], self._x[:, :, -2], y, z ) \
    ...
    def _batch_norm(self, x, affine=True, name='batch_norm'):
        ...
        with tf.variable_scope(name):
            ...
            self._extra_train_ops.append( moving_averages.assign_moving_average(moving_mean, mean, MOMENTUM))
            self._extra_train_ops.append( moving_averages.assign_moving_average(moving_variance, variance, MOMENTUM))
