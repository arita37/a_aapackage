...
class StockTrader:
    ...
    def update_summary(self, loss, r, q_value, actor_loss, w, p):
        ...
        self.w_history.extend(
            [",".join([str(Decimal(str(w0)).quantize(Decimal("0.00"))) for w0 in w.tolist()[0]])]
        )
        self.p_history.extend(
            [",".join([str(Decimal(str(p0)).quantize(Decimal("0.000"))) for p0 in p.tolist()])]
        )
    ...
    if args["mode"] == "train":
        if not os.path.exists(PATH_prefix):
            ...
            train_start_date, train_end_date, codes = (
                datetime.datetime.strptime(dict_data["train_start_date"], "%Y-%m-%d"),
                datetime.datetime.strptime(dict_data["train_end_date"], "%Y-%m-%d"),
                dict_data["codes"],
            )
            env.get_data(train_start_date, train_end_date, features, window_length, market, codes)
        ...
        test_start_date, test_end_date, codes = (
            datetime.datetime.strptime(dict_data["test_start_date"], "%Y-%m-%d"),
            datetime.datetime.strptime(dict_data["test_end_date"], "%Y-%m-%d"),
            dict_data["codes"],
        )
