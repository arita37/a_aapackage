...
class PG:
    def __init__(self, M, L, N, name, load_weights, trainable, type, number):
        ...
        self.pv_vector = (
            tf.reduce_sum(self.out * self.future_price, reduction_indices=[1]) * self.pc()
        )
    ...
    def build_net(self):
        state = tf.placeholder(
            tf.float32, shape=[None] + [self.M] + [self.L] + [self.N], name="market_situation"
        )
        network = tflearn.layers.conv_2d(state, 2, [1, 2], [1, 1, 1, 1], "valid", "relu")
        width = network.get_shape()[2]
        network = tflearn.layers.conv_2d(
            network, 48, [1, width], [1, 1], "valid", "relu", regularizer="L2", weight_decay=5e-9
        )
    ...
    def write_summary(self, reward):
        summary_str = self.sesson.run(self.summary_ops, feed_dict={self.summary_vars[0]: reward})
