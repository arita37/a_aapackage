...
def con2d(x, scope, trainable):
    with tf.variable_scope(scope):
        con_W_1 = tf.Variable(
            tf.truncated_normal([1, 3, int(x.shape[3]), 2], stddev=0.5), trainable=trainable
        )
        ...
        con_W_2 = tf.Variable(
            tf.truncated_normal([1, int(x.shape[2]), int(x.shape[3]), 48], stddev=0.5),
            trainable=trainable,
        )
        ...
        con_W_3 = tf.Variable(
            tf.truncated_normal([1, int(x.shape[2]), 48, 1], stddev=0.5), trainable=trainable
        )
...
def dense(x, out_dim, activation, scope, trainable):
    with tf.variable_scope(scope):
        t1_w = tf.Variable(
            tf.truncated_normal([int(x.shape[1]), out_dim], stddev=0.1), trainable=trainable
        )
    ...
    def __init__(self, predictor, M, L, N, name, load_weights, trainable):
        ...
        with tf.variable_scope("critic"):
            ...
            C_learning_rate = tf.train.exponential_decay(
                C_LR, global_step, decay_steps=2000, decay_rate=0.9, staircase=False
            )
            self.ctrain_op = tf.train.GradientDescentOptimizer(C_learning_rate).minimize(
                self.closs, global_step=global_step
            )
        ...
        with tf.variable_scope("sample_action"):
            self.sample_op = pi.sample(1)[
                0
            ]  # tf.squeeze(pi.sample(1),axis=[1,3])       # choosing action
        ...
        with tf.variable_scope("loss"):
            ...
            if METHOD["name"] == "kl_pen":
                ...
                self.aloss = -tf.reduce_mean(
                    tf.minimum(
                        surr,
                        tf.clip_by_value(ratio, 1.0 - METHOD["epsilon"], 1.0 + METHOD["epsilon"])
                        * self.tfadv,
                    )
                )
        ...
        with tf.variable_scope("atrain"):
            A_learning_rate = tf.train.exponential_decay(
                A_LR, global_step, decay_steps=2000, decay_rate=0.9, staircase=False
            )
