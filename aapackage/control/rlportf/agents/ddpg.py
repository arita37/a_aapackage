...
def build_predictor(inputs, predictor, nes_num, scope, trainable):
    with tf.name_scope(scope):
        if predictor == "CNN":
            ...
            conv1_W = tf.Variable(
                tf.truncated_normal([1, L, N, 32], stddev=0.15), trainable=trainable
            )
...
class StockActor:
    def __init__(self, sess, predictor, M, L, N):
        ...
        self.inputs, self.out, self.previous_action = self.build_actor(
            predictor, self.scopes[0], True
        )
        self.target_inputs, self.target_out, self.target_previous_action = self.build_actor(
        ...
        )  # list(map(lambda x: tf.div(x, self.batch_size), self.unnormalized_actor_gradients))#self.unnormalized_actor_gradients#list(map(lambda x: tf.div(x, 64), self.unnormalized_actor_gradients))
        ...
        self.pre_optimize = tf.train.AdamOptimizer(10e-3).minimize(
            self.pre_loss, global_step=global_step
        )
    ...
    def build_actor(self, predictor, scope, trainable):
        with tf.name_scope(scope):
            inputs = tf.placeholder(
                tf.float32, shape=[None] + [self.M] + [self.L] + [self.N], name="input"
            )
    ...
    def pre_train(self, s, a):
        pre_loss, _, _ = self.sess.run(
            [self.pre_loss, self.out, self.pre_optimize],
    ...
    def predict(self, inputs, a_previous):
        return self.sess.run(
            self.out, feed_dict={self.inputs: inputs, self.previous_action: a_previous}
        )
...
class StockCritic:
    def __init__(self, sess, predictor, M, L, N):
        ...
        self.inputs, self.actions, self.out, self.previous_action = self.build_critic(
            predictor, self.scopes[0], True
        )
        ...
        learning_rate = tf.train.exponential_decay(
            self.learning_rate, global_step, decay_steps=1000, decay_rate=0.90, staircase=False
        )

        self.optimize = tf.train.GradientDescentOptimizer(self.learning_rate).minimize(
    ...
    def build_critic(self, predictor, scope, trainable):
        with tf.name_scope(scope):
            ...
            out = tf.layers.dense(
                net, 1, kernel_initializer=tf.random_uniform_initializer(-0.003, 0.003)
            )

        return states, actions, out, actions_previous

    def train(self, inputs, actions, predicted_q_value, a_previous):
        critic_loss, q_value, _ = self.sess.run(
            [self.loss, self.out, self.optimize],
            feed_dict={
                self.inputs: inputs,
...
def build_summaries():
    critic_loss = tf.Variable(0.0)
    reward = tf.Variable(0.0)
    ep_ave_max_q = tf.Variable(0.0)
...
class DDPG:
    ...
    def train(self, method, epoch):
        ...
        target_q = self.critic.preditc_target(
            s_next, self.actor.predict_target(s_next, a_previous), a_previous
        )
