...
def parse_args2(ppa=None):
    ...
    ppa.add_argument(
        "--save_dir", type=str, default="", help="directory to store checkpointed models"
    )
    ...
    ppa.add_argument(
        "--model2", type=str, default="lstmCell", help="rnn, gru, lstm, gridlstm, gridgru"
    )
    ppa.add_argument(
        "--model3", type=str, default="lstmCell", help="rnn, gru, lstm, gridlstm, gridgru"
    )

    ppa.add_argument(
        "--n_hidden", type=str, default=100, help="hidden units in the recurrent layer"
    )  # 100
    ...
    ppa.add_argument("--grad_clip", type=float, default=5.0, help="clip gradients at this value")
...
class TextLoader(object):
    ...
    def create_batches(self):
        ...
        validation_batches = int(self.num_batches * 0.2)
        self.val_batches = zip(
            self.x_batches[-validation_batches:], self.y_batches[-validation_batches:]
        )
...
def visualize_result():
    ...
    for i, (k, v) in enumerate(files):
        train_loss = (
            pd.read_csv("./save/tinyshakespeare/{}".format(v)).groupby("epoch").mean()["train_loss"]
        )
        plt.plot(
            train_loss.index.tolist(), train_loss.tolist(), label=k, lw=2, color=tableau20[i * 2]
        )
    plt.legend()
    plt.xlabel("Epochs")
