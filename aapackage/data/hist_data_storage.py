...
class dailyDataStore(object):
    ...
    def download_todb(self, sym_list, table1='daily', start1='20140120', end1='20160805'):
        ...
        for i, sub_list in enumerate(self._break_list_to_sub_list(sym_list)):
            ...
            if self.source== 'yahoo' :  qlist, sym = pf.imp_yahoo_getquotes(sub_list, start=start1, end=end1)
            if self.source== 'google':  qlist, sym = pf.imp_googleQuoteList(sub_list, start=start1, end=end1)
            ...
            for k,df in enumerate(qlist) :
                ...
                df.to_sql(table1, self.con, if_exists='append', index=False, index_label=None, chunksize=None)
            del qlist, sym
    ...
    def get_sql(self, sqls='SELECT * FROM daily_us_stock where sym in ("SPY") order by date  asc', stock_list=[], table1='daily_us_etf',
                start1="", end1="", split_df=1):
        """ Retrieved a list of stocks covering the target date range select_all (bool):  Will pull all the stock symbol"""
        sym_str = ''.join(['"' + n + '",' for n in stock_list]); sym_str = sym_str[:-1]

        df = pd.read_sql_query(sql_str, self.con)
        df.reset_index(drop=True, inplace=True)
        df['date'] = df['date'].map(lambda x: int(x.replace('-', '')))  # in int value
        if start1 != "" and end1 != "" :  df= df[ (df['date'].values >= int(start1) )& (df['date'].values <= int(end1)) ]  #Date Interval creation

        if split_df  :
            qlist, sym= self._splitdf_inlist(df);   return qlist, sym
        else :
    ...
    def get_histo(self, stock_list=[], table1='daily_us_etf', start1="", end1="", split_df=1):
        ...
        if len(stock_list) > 0 :  sql_str = "SELECT * FROM %s where sym in (%s) order by date  asc  " % (table1, sym_str)
        else :                    sql_str = "SELECT * FROM %s order by date asc  " % (table1)
    ...
    def update_table(self, table1, colname):
        ...
        nchunk = 500000;   nn= len(df.index)
        for j in range(0, int(nn/nchunk)+1 ) :
            i = nchunk * j
            df2 = df[i:i + nchunk]
            df2.to_sql(table1, self.con, if_exists='append', index=False, index_label=None, chunksize=None)


    def clean_table(self, table1):
        print('Remove Duplicate')
        sql= "DELETE FROM "+table1+" WHERE rowid NOT  IN(SELECT  min(rowid) FROM "+ table1 +" GROUP BY sym, date )"
...
class intradayDataStore(object):
    def __init__(self, db_full_path='D:/_devs/Python01/project27/aaserialize/store/yahoo.db', source='yahoo'):
    ...
    def get_histo(self, stock_list=[], table1='daily_us_etf', start1="", end1="", split_df=1):
        ...
        if len(stock_list) > 0 :
            sql_str = "SELECT * FROM %s where sym in (%s) order by date  asc  " % (table1, stock_sym_str)
            df= pd.read_sql_query(sql_str ,  self.con)
        else :
        ...
        if split_df== 1 :
            ...
            l1 = []
            for key in UniqueNames:  l1.append(df[:][df.sym == key])
            return l1, UniqueNames
        else :
            return df
...
class riskDataStore(object):
    def __init__(self, db_full_path='D:/_devs/Python01/project27/aaserialize/store/risk1.db', listtable= ['daily','dividend']):
