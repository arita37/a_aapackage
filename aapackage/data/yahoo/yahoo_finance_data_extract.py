...
class YFinanceDataExtr(object):
    ...
    def __init__(self):
        ...
        self.cur_quotes_parm_headers = [
            ...
            "YEAR_HIGH",
            "YEAR_LOW",
        ]  # label to be use when downloading.
        ...
        self.properties_excel_table = (
            r"C:\pythonuserfiles\yahoo_finance_data_extract\Individual_stock_query_property.xls"
        )
    ...
    def form_cur_quotes_property_url_str_fr_excel(self):
        ...
        target_properties = "".join(
            [n[0].encode().strip() for n in self.xls_property_data.data_value_list]
        )
    ...
    def form_url_str(self, type="cur_quotes"):
        ...
        if type == "cur_quotes":
            ...
            self.cur_quotes_full_url = (
                self.cur_quotes_start_url
                + self.cur_quotes_stock_portion_url
                + self.cur_quotes_property_portion_url
                + self.cur_quotes_end_url
            )
    ...
    def cur_quotes_create_dataframe(self):
        ...
        self.cur_quotes_df.rename(
            columns={
                org: change.upper()
                for org, change in zip(self.cur_quotes_df.columns, self.cur_quotes_parm_headers)
            },
            inplace=True,
        )
    ...
    def rm_percent_symbol_fr_cols(self):
        ...
        for col in col_with_percent:
            self.temp_full_data_df[col] = self.temp_full_data_df[col].map(
...
if __name__ == "__main__":
    ...
    if choice == 1:
        ...
        data_ext.cur_quotes_parm_headers = [
            "NAME",
            "SYMBOL",
            "LATEST_PRICE",
            "OPEN",
    ...
    if choice == 4:
        ...
        data_ext.enable_form_properties_fr_exceltable = 0
        data_ext.set_full_stocklist_to_retrieve(
            ["%5EVIX", "%5EGSPC", "%5ESTI", "%5EDJI", "%5EIXIC", "%5EHSI", "%5EN225"]
        )
        data_ext.set_quotes_properties("nsl1op2kj")
        print(data_ext.cur_quotes_property_portion_url)
        data_ext.set_column_headers(
            ["NAME", "SYMBOL", "LATEST_PRICE", "OPEN", "ChangeInPercent", "YEAR_HIGH", "YEAR_LOW"]
        )
