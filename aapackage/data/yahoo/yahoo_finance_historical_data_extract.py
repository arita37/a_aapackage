...
class YFHistDataExtr(object):
    ...
    def __init__(self):
        ...
        self.date_interval = (
            10
        )  # the number of dates to retrieve, temp  default to 1 day per interval
        self.bypass_data_downloading = (
            0
        )  # bypass the downloading of data --> enable calling from database
        ...
        self.hist_quotes_date_dividend_portion_url = (
            ""
        )  # dividend part (combined with the interval portion)
        ...
        self.hist_quotes_csvfile_path = (
            os.getcwd() + "/aapackage/data/temp/raw_stock_data.csv"
        )  # for storing of all stock raw data
        self.tempfile_sav_location = (
            os.getcwd() + "/aapackage/data/temp/temp_hist_div_data_save.csv"
        )
        ...
        self.processed_interval = (
            3
        )  # set the period in which to process the post processing. For the filter most recert stock data.
    ...
    def calculate_start_and_end_date(self):
        ...
        start_date_tuple = (
            datetime.date.today() - datetime.timedelta(self.date_interval)
        ).timetuple()[0:3]
    ...
    def form_hist_quotes_date_interval_portion_url(self):
        ...
        from_date_url_str = "&c=%s&a=%s&b=%s" % (
            start_date_tuple[0],
            start_date_tuple[1] - 1,
            start_date_tuple[2],
        )
        end_date_url_str = "&f=%s&d=%s&e=%s" % (
            end_date_tuple[0],
            end_date_tuple[1] - 1,
            end_date_tuple[2],
        )
        ...
        self.hist_quotes_date_interval_portion_url = (
            from_date_url_str + end_date_url_str + interval_str
        )
        self.hist_quotes_date_dividend_portion_url = (
            from_date_url_str + end_date_url_str + dividend_str
        )
    ...
    def downloading_csv(self, download_type="hist"):
        ...
        if download_type == "hist":
            ...
            sav_filename = os.path.join(
                self.hist_quotes_csvfile_path,
                "hist_stock_price_" + self.individual_stock_sym + ".csv",
            )
        elif download_type == "div":
            target_url = self.div_history_full_url
            sav_filename = os.path.join(
                self.hist_quotes_csvfile_path, "div_hist_" + self.individual_stock_sym + ".csv"
            )
        else:
    ...
    def save_stockdata_to_df(self, download_type="hist"):
        ...
        if download_type == "hist":
            ...
            if len(self.all_stock_div_hist_df) == 0:
                ...
                self.all_stock_div_hist_df = self.all_stock_div_hist_df.append(
                    self.hist_quotes_individual_df
                )
    ...
    def get_hist_data_of_all_target_stocks(self):
        ...
        for stock in self.all_stock_sym_list:
            if self.print_current_processed_stock:
    ...
    def filter_most_recent_stock_data(self):
        ...
        self.processed_data_df = self.processed_data_df.groupby("SYMBOL").head(
            self.processed_interval
        )
    ...
    def get_trend_of_last_3_days(self):
        ...
        """
        self.processed_data_df = self.processed_data_df[
            self.processed_data_df["SYMBOL"].isin(
                self.stock_with_at_least_3_entries(self.processed_data_df)
            )
        ]
        grouped_symbol = self.processed_data_df.groupby("SYMBOL")
        falling_data = (
            (grouped_symbol.nth(2)["Adj Close"] >= grouped_symbol.nth(1)["Adj Close"])
            & (grouped_symbol.nth(1)["Adj Close"] >= grouped_symbol.nth(0)["Adj Close"])
            & (~(grouped_symbol.nth(2)["Adj Close"] == grouped_symbol.nth(0)["Adj Close"]))
        )  # check flat

        falling_df = (
            falling_data.to_frame().rename(columns={"Adj Close": "Trend_3_days_drop"}).reset_index()
        )

        rising_data = (
            (grouped_symbol.nth(0)["Adj Close"] >= grouped_symbol.nth(1)["Adj Close"])
            & (grouped_symbol.nth(1)["Adj Close"] >= grouped_symbol.nth(2)["Adj Close"])
            & (~(grouped_symbol.nth(2)["Adj Close"] == grouped_symbol.nth(0)["Adj Close"]))
        )  # check flat
        rising_df = (
            rising_data.to_frame().rename(columns={"Adj Close": "Trend_3_days_rise"}).reset_index()
        )
    ...
    def insert_yr_mth_col_to_div_df(self):
        ...
        """
        self.all_stock_div_hist_df["Div_year"] = self.all_stock_div_hist_df["Date"].map(
            lambda x: int(x[:4])
        )
        self.all_stock_div_hist_df["Div_mth"] = self.all_stock_div_hist_df["Date"].map(
            lambda x: int(x[6:7])
        )
    ...
    def insert_dividend_quarter(self):
        ...
        self.all_stock_div_hist_df["Div_1stQuarter"] = self.all_stock_div_hist_df["Div_mth"].isin(
            [1, 2, 3]
        )
        self.all_stock_div_hist_df["Div_2ntQuarter"] = self.all_stock_div_hist_df["Div_mth"].isin(
            [4, 5, 6]
        )
        self.all_stock_div_hist_df["Div_3rdQuarter"] = self.all_stock_div_hist_df["Div_mth"].isin(
            [7, 8, 9]
        )
        self.all_stock_div_hist_df["Div_4thQuarter"] = self.all_stock_div_hist_df["Div_mth"].isin(
            [10, 11, 12]
        )
    ...
    def get_dividend_payout_quarter_df(self):
        ...
        target_div_hist_df = self.all_stock_div_hist_df[
            (self.all_stock_div_hist_df["Div_year"] == curr_yr - 1)
        ]
        ...
        target_div_hist_df = (
            target_div_hist_df.groupby("SYMBOL")
            .agg(check_availiable1)
            .reset_index()[
                ["SYMBOL", "Div_1stQuarter", "Div_2ntQuarter", "Div_3rdQuarter", "Div_4thQuarter"]
            ]
        )
        self.all_stock_consolidated_div_df = pandas.merge(
            self.all_stock_consolidated_div_df, target_div_hist_df, on="SYMBOL", how="left"
        )
    ...
    def get_num_div_payout_per_year(self):
        ...
        target_div_hist_df = self.all_stock_div_hist_df[
            ~(self.all_stock_div_hist_df["Div_year"] == curr_yr)
        ]
        ...
        div_payout_df = (
            div_cnt_df.groupby("SYMBOL")
            .agg("mean")
            .reset_index()[["SYMBOL", "Dividends"]]
            .rename(columns={"Dividends": "NumDividendperYear"})
        )
        ...
        div_cnt_yr_basis_df = (
            div_cnt_df.groupby("SYMBOL")
            .agg("count")
            .reset_index()[["SYMBOL", "Div_year"]]
            .rename(columns={"Div_year": "NumYearPayin4Yr"})
        )

        ## join the data frame
        self.all_stock_consolidated_div_df = pandas.merge(
            div_payout_df, div_cnt_yr_basis_df, on="SYMBOL"
        )
    ...
    def get_prev_3rd_yr_df(self):
        ...
        prev_3rd_yr_df = (
            target_div_hist_df.groupby("SYMBOL")
            .agg("mean")
            .reset_index()[["SYMBOL", "Adj Close"]]
            .rename(columns={"Adj Close": "Pre3rdYear_avg"})
        )
    ...
    def run_all_hist_data(self):
        ...
        self.all_stock_combined_post_data_df = pandas.merge(
            self.price_trend_data_by_stock,
            self.all_stock_consolidated_div_df,
            on="SYMBOL",
            how="left",
        )
        self.all_stock_combined_post_data_df = pandas.merge(
            self.all_stock_combined_post_data_df, prev_3rd_yr_df, on="SYMBOL", how="left"
        )
...
if __name__ == "__main__":
    ...
    if choice == 2:
        ...
        falling_data = (
            grouped_symbol.nth(0)["Adj Close"] >= grouped_symbol.nth(1)["Adj Close"]
        ) >= grouped_symbol.nth(2)["Adj Close"]
