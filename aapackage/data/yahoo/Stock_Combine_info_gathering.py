...
def get_filename(dir_path, filename_prefix, offset_to_cur_date=0, file_ext=".csv"):
    ...
    """
    return os.path.join(
        dir_path, filename_prefix + set_last_desired_date(offset_to_cur_date) + file_ext
    )
...
if __name__ == "__main__":
    ...
    if choice == 1:
        ...
        if "c_backup" in partial_run:
            ...
            full_stock_data_df = pandas.merge(
                ss.modified_df, trend_ext.all_stock_combined_post_data_df, on="SYMBOL", how="left"
            )
        ...
        if "d_pre" in partial_run:
            ...
            full_stock_data_df = pandas.merge(
                full_stock_data_df, com_data_df, on="SYMBOL", how="left"
            )
        ...
        if "d" in partial_run:
            ...
            full_stock_data_df = pandas.merge(
                full_stock_data_df, dd.com_data_allstock_df, on="SYMBOL", how="left"
            )
        ...
        if "e" in partial_run:
            ...
            ## will have to merge the two data set
            full_stock_data_df = pandas.merge(
                full_stock_data_df, w.processed_histdata_combined, on="SYMBOL", how="left"
            )
        ...
        if "f" in partial_run:
            ...
            full_stock_data_df = set_column_sequence(
                full_stock_data_df,
                [
                    "CompanyName",
                    "SYMBOL",
