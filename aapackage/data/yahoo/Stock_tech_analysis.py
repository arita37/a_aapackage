...
class TechAnalysisAdd(object):
    def __init__(self, stocklist):
        ...
        ## user parameters
        self.get_fr_database = (
            0
        )  # 1 will pull data from database, 0 will get directly get from web.
    ...
    def add_Bollinger_parm(self):
        ...
        temp_data_set = self.histdata_indiv_stock.sort(
            "Date", ascending=True
        )  # sort to calculate the rolling mean
        ...
        ) + 2 * pandas.rolling_std(temp_data_set["Adj Close"], 20, min_periods=20)
        temp_data_set["Bol_lower"] = pandas.rolling_mean(
            temp_data_set["Adj Close"], window=20
        ) - 2 * pandas.rolling_std(temp_data_set["Adj Close"], 20, min_periods=20)
        temp_data_set["Bol_BW"] = (
            (temp_data_set["Bol_upper"] - temp_data_set["Bol_lower"]) / temp_data_set["20d_ma"]
        ) * 100
        temp_data_set["Bol_BW_200MA"] = pandas.rolling_mean(
            temp_data_set["Bol_BW"], window=50
        )  # cant get the 200 daa
    ...
    def bollinger_plots(self):
        ...
        """
        self.histdata_indiv_stock.plot(
            x="Date",
            y=["Adj Close", "20d_ma", "50d_ma", "Bol_upper", "Bol_lower", "Pivot", "S1", "R1"],
        )
    ...
    def get_most_current_dataset(self):
        ...
        self.processed_histdata_combined = (
            self.histdata_combined.groupby("SYMBOL").first().reset_index()
        )
...
if __name__ == "__main__":
    ...
    if 5 in choice:
        ...
        t = data_ext.temp_full_data_df[
            ["SYMBOL", "NAME", "OPEN", "PERATIO", "PRICEBOOK", "DILUTEDEPS"]
        ]
        print(t)
