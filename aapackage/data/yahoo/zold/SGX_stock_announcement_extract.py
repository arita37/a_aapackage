...
class SGXDataExtract(WebJsonRetrieval):
    ...
    def process_all_data(self):
        ...
        """
        for sgx_item in list(self.retrieval_dict.keys()):
    ...
    def joined_curr_to_company_info_data(self):
        ...
        self.sgx_curr_plus_company_df = pandas.merge(
            self.sgx_curr_price_df, stock_info_df, left_on="SYMBOL", right_on="tickerCode"
        )

        # change some of the symbol to match the stock combine data set.
        self.sgx_curr_plus_company_df = self.sgx_curr_plus_company_df.rename(
            columns={
                "peRatio": "PERATIO",
    ...
    def joined_relevent_sgx_data(self):
        ...
        stockcom_info_df["companyName"] = stockcom_info_df["companyName"].apply(
            lambda x: x.encode().upper()
        )
    ...
    def filter_key_announcement(self, list_of_stock):
        ...
        """
        filtered_announc_df = self.sgx_announ_df[
            self.sgx_announ_df["tickerCode"].isin(list_of_stock)
        ].dropna()

        ##shortened version
        sh_filtered_announc_df = filtered_announc_df[["SecurityName", "CategoryName", "AnnTitle"]]
        sh_filtered_announc_df["AnnTitle"] = sh_filtered_announc_df["AnnTitle"].apply(
            lambda x: x[:20]
        )
    ...
    def retrieve_curr_price_df(self):
        ...
        for parameter in ["DailyVolume", "BuyVolume", "SellVolume"]:
            self.sgx_curr_price_df[parameter] = (
                self.sgx_curr_price_df[parameter] * 1000
            )  # convert to actual shares

        ## new parameters
        self.sgx_curr_price_df["buy_to_sell_ratio"] = self.sgx_curr_price_df["BuyVolume"] / (
            self.sgx_curr_price_df["SellVolume"] + 1
        )  # plus one to make it non zero
    ...
    def retrieve_shortsell_info(self):
        ...
        for last_effective_date in range(7):
            ...
            if len(self.shortsell_info_df) == 0:
                continue

            self.shortsell_info_df.rename(
                columns={
                    0: "Security",
                    1: "Short Sale Volume",
                    2: "Currency",
            ...
            ] = self.shortsell_info_df[["Short Sale Volume", "Short Sale Value"]].astype(float)
            # need a rank on the short sell
            self.shortsell_info_df["ranked_shortsell"] = self.shortsell_info_df[
                "Short Sale Volume"
            ].rank(method="min", ascending=False)
            self.shortsell_info_df["shortsell_lastdate"] = self.set_last_desired_date(
                last_effective_date
            )
    ...
    def form_shortsell_url(self, last_effective_date):
        ...
        self.shortsell_full_url = (
            self.shortsell_info_start_url + self.shortsell_date_url + self.shortsell_end_url
        )
    ...
    def set_last_desired_date(self, num_days=0):
        ...
        """
        last_eff_date_list = list(
            (datetime.date.today() - datetime.timedelta(num_days)).timetuple()[0:3]
        )
    ...
    def shortsell_notification(self):
        ...
        merged_shortsell_df = pandas.merge(
            self.shortsell_info_df,
            self.sgx_curr_price_df,
            left_on="Security",
            right_on="CompanyName",
        )

        ## add in additional columns
        merged_shortsell_df["shortsell_vol_per"] = (
            merged_shortsell_df["Short Sale Volume"] / merged_shortsell_df["DailyVolume"]
        )
        merged_shortsell_df["ranked_percent_vol_shortsell"] = merged_shortsell_df[
            "shortsell_vol_per"
        ].rank(method="min", ascending=False)

        top_shortsell_df = merged_shortsell_df[
            merged_shortsell_df["ranked_shortsell"].isin(list(range(1, 16)))
        ]
        ...
        tar_watchlist_shortsell_df = merged_shortsell_df[
            merged_shortsell_df["Security"].isin(self.companyname_watchlist)
        ]
        tar_watchlist_shortsell_df = tar_watchlist_shortsell_df[
            [
                "Security",
                "Short Sale Volume",
                "ranked_shortsell",
                "shortsell_vol_per",
                "ranked_percent_vol_shortsell",
            ]
        ]
        tar_watchlist_shortsell_df = tar_watchlist_shortsell_df[
            tar_watchlist_shortsell_df["ranked_shortsell"].isin(list(range(1, 100)))
        ]
        tar_watchlist_shortsell_df = tar_watchlist_shortsell_df.sort(
            columns="ranked_shortsell", ascending=True
        )
        ...
        if tar_watchlist_shortsell_shtver:
            p.pushNote(
                "all",
                "Shortsell targetwatchlist",
                tar_watchlist_shortsell_shtver,
                recipient_type="random1",
            )
    ...
    def scan_price_limit_alert(self):
        ...
        for stockname, price_target, criteria in self.price_limit_reach_watchlist:
            ...
            if criteria == "greater":
                if (
                    target_df["LastPrice"].values[0] >= price_target
                ):  # assume only one data returned
                    store_the_result = 1
            elif criteria == "lower":
                if (target_df["LastPrice"].values[0] > 0) and (
                    target_df["LastPrice"].values[0] <= price_target
                ):  # assume only one data returned
                    store_the_result = 1
            ...
            if store_the_result == 1:
                if len(self.price_limit_alerts_df) < 1:
                    ...
                    self.price_limit_alerts_df = pandas.concat(
                        [self.price_limit_alerts_df, target_df]
                    )
    ...
    def retrieve_and_notify(self):
        ...
        if self.filtered_announ_shtver:
            p.pushNote(
                "all", "Announcement Info", self.filtered_announ_shtver, recipient_type="random1"
            )
        ...
        if len(self.price_limit_alerts_df) > 0:
            p.pushNote(
                "all",
                "Price alert",
                self.price_limit_alerts_df.to_string(),
                recipient_type="random1",
            )
...
def price_stream_alert():
    ...
    if len(w.price_limit_alerts_df) > 0:
        p.pushNote(
            "all", "Price alert", w.price_limit_alerts_df.to_string(), recipient_type="random1"
        )
...
if __name__ == "__main__":
    ...
    if choice == 8:
        ...
        shortsell_df.rename(
            columns={0: "Security", 1: "Short Sale Volume", 2: "Currency", 3: "Short Sale Value"},
            inplace=True,
        )
