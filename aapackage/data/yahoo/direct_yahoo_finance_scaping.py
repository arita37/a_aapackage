...
class YFinanceDirectScrape(object):
    ...
    def __init__(self):
        ...
        self.stock_sym_append_str = (
            ".SI"
        )  # default ".SI", extra parameters to append to the stock symbol.
    ...
    def get_list_of_param_selector_avaliable(self):
        ...
        """
        print(list(self.start_url_dict.keys()))
    ...
    def set_param_selector(self, param_type):
        ...
        """
        if param_type not in list(self.start_url_dict.keys()):
    ...
    def form_full_url(self):
        ...
        """
        assert self.param_selector in list(self.start_url_dict.keys())
    ...
    def __dom_object_isempty(self, dom_object):
        ...
        """
        if len(dom_object) == 0:
            if self.__print_parsing_problem:
    ...
    def parse_all_parameters(self):
        ...
        """
        for n in list(self.start_url_dict.keys()):
            if self.__print_individual_stock_data:
                print("Parsing parameters: ", n)
            self.set_param_selector(n)
            self.form_full_url()
            if self.print_url_str:
                print(self.full_url_str)
            self.parse_method_dict[n]()
            if self.url_query_timeout:
                return
    ...
    def create_individual_stock_df(self):
        ...
        self.individual_stock_df.rename(
            columns={
                org: change
                for org, change in zip(list(range(len(self.value_list))), self.header_list)
            },
            inplace=True,
        )
    ...
    def parse_company_desc(self):
        ...
        if self.url_query_timeout:
            return
        dom_object = self.tag_element_results(
            self.dom_object, self.css_selector_dict[self.param_selector]
        )
    ...
    def parse_one_level_header_value_set(self, parse_start, parse_end):
        ...
        dom_object = self.tag_element_results(
            self.dom_object, self.css_selector_dict[self.param_selector][0]
        )
        if self.__dom_object_isempty(dom_object):
            return
        ...
        # value
        dom_object = self.tag_element_results(
            self.dom_object, self.css_selector_dict[self.param_selector][1]
        )
        for n in dom_object[parse_start:parse_end]:
            try:
