...
def mapping_calc_risk_v02(ss, tr, t, risk0):
    ...
    if (
        (ss[t, 0] < -13.0 and ss[t, 2] < -18.0)
        or (ss[t, 0] < -9.0 and ss[t, 2] < -26.0)
        or (ss[t, 0] < -5.0 and ss[t, 2] < -35.0)
        or (risk0 == 6 and ss[t, 0] < -8.0 and ss[t, 2] < -16.0)
    ):
        risk = 6.0

    # Crash Protection
    elif ((ss[t, 0] <= 0.001 and ss[t - 1, 0] >= ss[t, 0] - 0.01) or ss[t, 0] <= 0.2) and 0.333 * (
        ss[t - 2, 1] + ss[t - 1, 1] + ss[t, 1]
    ) < 3.5:
        ...
        # Drawdown reduces
        elif (risk0 == 0.0 or risk0 == 6) and ss[t, 2] > 0.333 * (
            ss[t - 1, 2] + ss[t - 2, 2] + ss[t - 3, 2]
        ) - 1.5:
            risk = 0.0

        elif ss[t, 2] > -1.5:  # Drawdown is too small, do NOT move to Bear Protection
            risk = risk0
    ...
    elif (risk0 == 4.0 or risk0 == 2.0 or risk0 == 0.0 or risk0 == 6.0) and (
        (ss[t, 2] < -2.5) and (ss[t - 1, 2] > -1.0 or ss[t - 2, 2] > -1.0)
    ):
        risk = 2.0
...
def mapping_calc_risk_v01(ss, tr, t, risk0):
    ...
    elif (ss[t, 0] <= 0.001 and ss[t - 1, 0] >= ss[t, 0] - 0.01) or ss[
        t, 0
    ] <= -1.0:  # Crash Protection
...
def mapping_risk_ww_v01(risk, wwmat, ww2):
    if risk == 6.0:
        ...
        ww2 = np.array([0.0, 0.1, 0.0, 0.2, 0.3, 0.0, 0.0, 0.0, 0.4])
    return ww2
