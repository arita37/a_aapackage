...
def date_align(quotes, dateref=None, datestart=19550101, type1="close"):
    ...
    if isnotint1:
        dateref = np.array(util.datetime_toint(dateref))
    ...
    for k in range(0, masset):
        ...
        if not isint(df.date.values[0]):
            datei = np.array(util.datetime_toint(df.date.values))
        else:
...
def _reshape(x):
    if len(np.shape(x)) < 2:
        return np.reshape(x, (1, np.size(x)))
    else:
        return x
...
def plot_price(
    ...
    if _notnone(y2):
        asset = np.concatenate((asset, _reshape(y2)))
    if _notnone(y3):
        asset = np.concatenate((asset, _reshape(y3)))
    if _notnone(y4):
        asset = np.concatenate((asset, _reshape(y4)))
    if _notnone(y5):
        asset = np.concatenate((asset, _reshape(y5)))
    ...
    if sh[0] == 1:
        ...
        if date1 is None:
            ...
            tick_lbls = date1[np.arange(0, int(tmax), tickperday)]
        plt.xticks(tick_locs, tick_lbls)
...
def plot_pricedate(
    ...
    if type(date1[0]) == str:
        date1 = datestring_todatetime(date1)
    ...
    if len(savename1) > 1:
        ax.savefig(savename1, dpi=150, bbox_inches="tight")
...
def save_asset_tofile(file1, asset1, asset2=None, asset3=None, date1=None, title1=None):
    ...
    if date1 != None:
        for t in range(0, tmax1):
            assetall[0, t] = int(date1[t])  # Date

    for t in range(0, tmax1):
        for k in range(0, masset1):
            assetall[1 + k, t] = asset1[k, t]  # 1st Asset

        if asset2 != None:
            for k in range(0, masset2):
                assetall[1 + masset1 + k, t] = asset2[k, t]

        if asset3 != None:
            for k in range(0, masset3):
                assetall[1 + masset2 + k, t] = asset3[k, t]

    if title1 != None:
        table1 = np.row_stack((title1, assetall.T))
    else:
        table1 = assetall.T
...
def correlation_mat(Xmat, type1="robust", type2="correl"):
    ...
    if type1 == "empirical2":
        ...
        correl = model.covariance_
        correl_inv = model.precision_
...
def cointegration(x, y):
  ...
  """
    ...
    res2[1, 0] = "Proba_CoInte_95%_confid"
    res2[1, 1] = res[0] < res[2][1]  # Proba
    res2[2, 0] = "Proba_CoInte_90%_confid"
    res2[2, 1] = res[0] < res[2][2]  # Proba
    return res2
...
def reg_slope(close, dateref, tlag, type1="elasticv"):
    ...
    for t0 in range(tlag, len(dateref)):
...
def rolling_cointegration(x, y):
    ...
    historical_coint = pd.expanding_apply(
        data.realgdp, rolling_coint, min_periods=36, args=(data.realdpi,)
    )
...
def regression(yreturn, xreturn, type1="elasticv"):
    ...
    if len(np.shape(yreturn)) < 2:
        yreturn = yreturn.ravel()
    if len(np.shape(xreturn)) < 2:
        xreturn = xreturn.reshape(-1, 1)
...
def regression_fixedsymbolstock(sym, ret_close2, tsstart, tsample, ret_spy, spyclose, regonly=True):
    ...
    if regonly:
        ...
        err = np.max(
            np.abs(spyclose[0, tsample : tsample + tahead] - spyreg[0, tsample : tsample + tahead])
        )
...
def regression_allstocks_vs_riskfactors(symstock, pricestock, symriskfac, priceriskfac, nlaglist):
    ...
    # Pandas Part
    colname = ["ticker", "lag", "R2"]
    colname = colname + symriskfac
    ...
    for ilag, nlag in enumerate(nlaglist):  # Time lag
        ...
        for k in range(0, len(stocklist01)):
            ...
            _, ww01, r2, _ = regression_fixedsymbolstock(
                sym_riskid, ret_close2, 0, tmax, ret_stockjp, None, regonly=True
            )
...
def rsk_calc_all_TA(df="panda_dataframe"):
    ...
    df = ta.MA(df, 5)
    df = ta.MA(df, 3)
    df = ta.distance(df, "MA_200")
    df = ta.distance(df, "MA_50")
    df = ta.distance(df, "MA_20")
    df = ta.distance(df, "MA_5")
    df = ta.distance(df, "MA_3")
    ...
    df = ta.nbday_low(df, 252)
    df = ta.nbday_high(df, 252)
    df = ta.RSI(df, 14)

    df = ta.qearning_dist(df)
...
def ta_lowbandtrend1(close2, type1=0):
    ...
    if len(vmax) > 2:
        ...
        vmax1 = vmax[kix1:kix0, 1]
        vmax0 = vmax[kix1:kix0, 0]
        res = sci.optimize.differential_evolution(
            linearreg2, [(-2.0, 2.0), (0, pmin1 * 3)], tol=0.1
        )
    else:
        res = [0.0, 0.0]
...
def ta_highbandtrend1(close2, type1=0):
    ...
    if len(vmax) > 2:
        ...
        vmax0 = vmax[kix1:kix0, 0]
        res = sci.optimize.differential_evolution(
            linearreg, [(-2.0, 2.0), (0.0, pmax1 * 3)], tol=0.1
        )
...
def calcbasket_table(wwvec, ret, type1="table", wwtype="constant", rebfreq=1, costbps=0.000):
    ...
    if type1 == "table":
        return bsk, wwall.T  # Return table of Price/weights


# Creation of RiskPa assets
def folio_createvolta_asset(close, vol=0.12, volrange=120, lev=1.0):
    closepa = np.zeros_like(close)
    masset, _ = np.shape(close)
    for i in range(0, masset):
        riskpa = folio_volta(close[i, :], vol, volrange, lev)
        closepa[i, :] = riskpa
    return closepa


def folio_lowcorrelation(
    sym01, nstock, periodlist, dateref, close1, kbenchmark, badlist, costbppa=0.02, showgraph=True
):
    t1list = periodlist
    nmax = nstock
    ...
    def ww_selection(nmax1, ret_close1, badlist, *args):
        ...
        for i in range(0, 100):
            ...
            if (
                not k in badlist
                and np.std(ret_close1[k, max(0, (t1 - 50)) : (t1)]) > 0.005
                and not np.isnan(np.sum(ret_close1[k, t1:(t2)]))
            ):  # No zero Volatility :
                stk_select = [k]
                break
        ...
        for ii in range(0, nmax - 1):  # 2nd stock select
            ...
            for k in range(0, nstock):
                if not k in badlist and util.find(k, stk_select) == -1:
                    ...
                    if scor < scormin:
                        if (
                            np.std(ret_close1[k, max(0, (t1 - 252)) : (t1)]) > 0.005
                            and np.sum(ret_close1[k, t1:(t2)]) != 0
                        ):  # No zero Volatility
                            scormin = scor
                            kmin = k
        ...
        ww = ww / sum(ww)
        if np.isnan(sum(ww)):
            ww = 1 / len(stk_select) + np.zeros(len(stk_select))
    ...
    for ss, t1 in enumerate(t1list):  # All sub-period
        ...
        stk_select2 = np.zeros_like(wwall)
        stk_select = np.array(stk_select)
        ...
        if showgraph:
            ...
            plot_price(
                ...
                sym=["BSK", "Benchmark"],
                tickperday=5,
                label=("Daily Variation ", "Weeks", "Variation in %"),
            )

            plot_price(
                bsk - bench1[0, :],
                bench1[0, :] - 100,
                sym=["L/S wth Benchmark", "Benchmark"],
                tickperday=5,
                label=("Daily Variation ", "Weeks", "Variation in %"),
            )
            ...
            for k in range(0, nmax1):
                print(
                    sym01[int(stk_select[k])][:] + " JP Equity ; ",
                    a_nk400name[util.find(sym01[stk_select[k]], a_nk400list)] + ";",
                    str(ww[k]) + ";",
                )
        ...
        print(
            (
                ...
                + str(dateref[t1])
                + "; Perf: "
                + str(bsk[-1] - bench1[0, -1])
                + "  ; NKPerf:"
                + str(bench1[0, -1] / bench1[0, 0] - 1)
            )
        )

        # --------------------- Risk  Storage ---------------------------------------------
        risktable[jj, 0] = dateref[t1]
        risktable[jj, 1] = dateref[t2]
        risktable[jj, 2] = bsk[-1]
        risktable[jj, 3] = bench1[0, -1]
        risktable[jj, 4] = bsk[-1] - 100
        risktable[jj, 5] = bench1[0, -1] - 100
        ...
        wwtable[jj, 0] = dateref[t1]
        wwtable[jj, 1] = dateref[t2]
    ...
    lsfull = np.zeros(len(bskfull))
    lsfull[0] = 100.0
    for t in range(1, len(bskfull)):
        lsfull[t] = (
            lsfull[t - 1]
            * (0.55 * bskfull[t] / bskfull[t - 1] + 0.45 * invbench[t] / invbench[t - 1])
            * (1 - costbppa / 252.0)
        )
...
def folio_longshort_unit(long1, short1, ww=None, costpa=0.0, tlag=1, istable=1, wwschedule=None):
    ...
    for t in range(0, tmax):
        ...
        # In % of the asset
        wwall[t, 3] = nl * long1[t] / folio[t]
        wwall[t, 4] = ns * short1[t] / folio[t]
...
def folio_longshort_unitfixed(long1, short1, nn=None, costpa=0.0, tlag=1, istable=1):
    ...
    for t in range(1, tmax):
        ...
        # In % of the asset
        wwall[t, 3] = nl * long1[t] / invetf[t]
        wwall[t, 4] = ns * short1[t] / invetf[t]
...
def folio_longshort_pct(long1, short1, ww=None, costpa=0.0):
    ...
    for t in range(1, len(long1)):
        invetf[t] = (
            invetf[t - 1]
            * (1 + ww[0] * (long1[t] / long1[t - 1] - 1) + ww[1] * (short1[t] / short1[t - 1] - 1))
            * (1 - costpa / 252.0)
        )
...
def folio_volta(
    ...
    wwvol = np.zeros((tmax, 2), dtype="float32")
...
def folio_fixedunitprice(price, fixedww, costpa=0.0):
    ...
    bsk = np.zeros(tmax, dtype="float32")
    bsk[0] = 100.0

    for t in range(1, tmax):
        retbk = 0
        for k in range(0, masset):
            retbk += fixedww[k] * (price[k, t] / price[k, 0] - 1)
...
def folio_fixedweightprice(price, fixedww, costpa=0.0):
    ...
    for t in range(1, tmax):
        retbk = 0
        for k in range(0, masset):
            retbk += fixedww[k] * (price[k, t] / price[k, t - 1] - 1)
...
def folio_cost_turnover(wwall, bsk, dateref, costbp):
    ...
    plot_price(
        nbrebal,
        date1=dateint_tostring(dateref, format1="%b-%y"),
        tickperday=242,
        sym=["Cumulative"],
        label=("Nb of rebalancing", "", ""),
        legendloc="upper left",
        isband=0,
    )
    ...
    plot_price(
        ...
        tickperday=242,
        sym=["Cumulative"],
        label=("Cumulative Transaction Cost in %", "", ""),
        legendloc="upper left",
        isband=0,
    )
    ...
    return {
        ...
        "nbrebal": nbrebal,
        "costpa": cost / years,
        "% TotalWeightchange pa": wsum2 / years,
        "Nb rebalancing pa": wcount1 / years,
    }


def folio_riskpa(ret, targetvol=0.1, volrange=90, cap=1.0):
    masset, tmax = np.shape(ret)
    bsk2 = np.zeros(tmax + 1, dtype="float32")
    bsk2[0] = 100
...
class folioSimulation(object):
    ...
    def setcriteria(self, lweight, name, initperiod):
        ...
        # basket details :
        self.amt = self.wwbasket["notional"]
        self.rebfreq = self.wwbasket["rebfreq"]
        self.costbp = self.wwbasket["costbp"]
        self.costfixedbp = self.wwbasket["costfixedbp"]
        self.costshortbp = self.wwbasket["costshortbp"]
        self.borrowshortbp = self.wwbasket["borrowshortbp"]
    ...
    def calc_baskettable_pct(self, type1="table", showdetail=0):
        ...
        bsk[0] = 1.0
        trebal = 0
        wwpct_actual = self.wwasset[0, :]
        wwpct_th = wwpct_actual
        ...
        for t in range(1, tmax + 1):
            ...
            if np.mod(t, rebfreq) == 0:  # rebal of weights
                ...
                hedgecost = (
                    np.sum(np.abs(wwpct_new - wwpct_actual) * bsk[t - 1]) * costbp
                )  # Cost 0.002
                wwpct_actual = wwpct_new
                trebal = t

            bsk[t] = (
                bsk[t - 1] * (1 + np.sum(wwpct_actual * ret[:, t - 1])) - hedgecost
            )  # ww basket
            self._udpate_wwindpct(t, bsk[t], hedgecost, wwpct_actual, wwpct_th)

            if showdetail:
                print(self.dateref[t], bsk[t], wwpct_actual)
    ...
    """
        ...
        bsk[0] = self.amt
        wwunit_actual = self.ww0unit
        wwunit_new = wwunit_actual
        ...
        for t in range(0, tmax):
            ...
            if np.mod(t, rebfreq) == 0 and t < tmax - 1:
                ...
                transact = np.sum(
                    (wwunit_new - wwunit_actual) * close[:, trebal]
                )  # wwactual ---> wwnew at (t+1)
                hedgecost = (
                    np.sum(np.abs(wwunit_new - wwunit_actual) * close[:, trebal]) * costbp
                )  # Cost 0.002
            ...
            self._udpate_wwind(
                t + 1, bsk, cash, transact, hedgecost, wwunit_actual, wwpct_actualinv
            )  # t---> t+1
        ...
        wwdiff[:, 1] = 100.0 * (
            self.wwindpct[:, 2] / self.wwindpct[0, 2] - self.wwind[:, 2] / self.wwind[0, 2]
        )
        wwdiff[:, nind : nind + masset] = (
            self.wwindpct[:, nind : nind + masset]
            - self.wwind[:, nind + masset : nind + 2 * masset]
        )
    ...
    def get_orderlist(self):
        order1 = np.zeros(
            (self.tmax, 6), dtype=np.object
        )  # dateint, ticker, units, unitprice, ordertype Buy/Sell

        order1 = []
        for t in range(0, self.tmax):
            if (
                np.sum(np.abs(self.wwind[t, self.nind + self.masset * 2 :])) > 5
            ):  # Only meanful rebalancing
                for k in range(0, self.masset):
    ...
    def _wwunit_rebal(
        self, bsk, t, trebal
    ):  #  wunit: t+1 ---->  t+2,     wwpct[t+2],  B[t+1], S[t+1
        wwpct_new = self.wwindpct[
            t + 2, self.nind : self.nind + self.masset
        ]  # at t+2, B[t+2] uses ww[t+2]
        wwunit_new = np.round(
            wwpct_new * (self.amt * self.wwindpct[t + 1, 2]) / self.close[:, t + 1], 0
        )  # B[t+1] uses  close[t
        return wwunit_new

    def _wwpct_tounit(self, wwpct, bskt, closet):
        return np.round((self.amt * bskt) / closet * wwpct, 0)  # Unitaire

    def _wwunit_topct(self, wwunit, bskt, closet):
        return wwunit / (self.amt * bskt) * closet

    def _udpate_wwind(self, t, bsk, cash, transact, hedgecost, wwunit_actual, wwpct_actualinv):
        masset = self.masset
        self.wwind[t, 1] = t
        self.wwind[t, 2] = bsk[t]
        self.wwind[t, 3] = cash  # cumulative
        self.wwind[t, 4] = transact
        self.wwind[t, 5] = hedgecost
        ...
        self.wwind[t, self.nind + 2 * masset : self.nind + 3 * masset] = (
            wwunit_actual - self.wwind[t - 1, self.nind : self.nind + masset]
        )  # Diff of Units
...
class folioCalc(object):
    def __init__(self, sym, close, dateref):
        ...
        if tmax + 1 != len(dateref):
            print("Dateref and Close length mistmatch")
    ...
    def _weightcalc_generic(self, wwvec, t):
        if self.wwtype == "constant":
            return self._weightcalc_constant(wwvec, t)
        if self.wwtype == "regime":
            return self._weightcalc_regime(wwvec, self.wwrisk, t)
        if self.wwtype == "regimeExtra":
            return self._weightcalc_regime(self.wwasset, wwvec, t)
    ...
    def _weightcalc_regime(self, wwvec, wwextra, t):
        if t < self.nbrange:
            ...
            wwmat = np.reshape(
                wwvec, (self.masset, self.nbregime)
            )  # Reshape into matrix asset x nbRegime
            ww2 = self._mapping_risk_ww(
                risk, wwmat, self.wwasset0
            )  # Market Cycle/Regime from Risk Indicator Level
            return ww2

    def _regimecalc(self, t, wwextra):  # Risk Indicator
        if self.riskid == "spprice":
            return self.riskind[0, t - 1]
        if self.riskid == "spperf":
            return 100.0 * (self.riskind[0, t - 1] / self.riskind[0, t - 1 - self.nbrange] - 1)
    ...
    # Calc Basket Values from Input data
    def calc_baskettable(
        self, wwvec, ret, type1="table", wwtype="constant", rebfreq=1, costbps=0.000, showdetail=0
    ):
        ...
        for t in range(1, tmax + 1):
            ...
            if np.mod(t, rebfreq) == 0:  # rebabacing of weights
                ...
                if costbps != 0:
                    hedgecost = np.sum(np.abs(ww2 - ww2i)) * bsk[t - 1] * costbps  # Cost 0.002
                ww2 = ww2i
    ...
    def plot(self, wwvec=None, show1=1, tickperday=60):
        if wwvec is None:
            wwvec = self.wwbest
        bsk, wwall = self.calc_baskettable(
            wwvec,
            self.ret,
            type1="table",
            wwtype=self.wwtype,
            rebfreq=self.rebfreq,
            costbps=self.costbp,
        )
    ...
    def multiperiod_ww(self, t):
        ...
        kdate = 0
        tt = self.dateref[t]
        for k in range(0, self.kmaxmodel - 1):
...
class folioRiskIndicator(object):
    ...
    """
        ...
        try:
            ...
            self.date_31dec = self.wwbasket["schedule1"]
            self.tlagsignal = self.wwbasket["tlagsignal"]
            self.nrisk = self.wwbasket["nrisk"]
        except:
            pass
    ...
    def _weightcalc_regime(self, wwvec, wwextra, t):
        if t < self.nbrange:
            ...
            self._regimecalc(t, wwextra)  # Risk indicator
    ...
    def _regimecalc(self, t, wwextra):  # Risk Indicator
        ...
        if self.riskid == "spperf":
            return 100.0 * (self.riskind[0, t - 1] / self.riskind[0, t - 1 - self.nbrange] - 1)
        if self.riskid == "multi":
            self.riskind_out[t, 1] = self._mapping_calc_risk(
                self.riskind, wwextra, t, self.riskind_out
            )

    def calc_optimal_weight(self, maxiter=1, name1="", isreset=1, popsize=15):
        fbest, xbest, solver = datanalysis.sk_optim_de(
            self.calcbasket_obj,
            self.boundsoptim,
            maxiter=maxiter,
            name1=name1,
        ...
        self.wwbest = xbest
        self.res = (copy.deepcopy(solver), xbest, fbest)
...
def calcbasket_objext(RETURN, TMAX, riskind_i, wwmat, wwasset0, ww0, nbrange, criteria):
    ...
    for t in range(1, TMAX + 1):
        ...
        if t < nbrange:
            ...
            risk = riskind_i[t, 1]
            if risk == 4.0:
                ww2 = wwmat[:, 2]  # Risk indicator
            elif risk == 6.0:
                ww2 = wwasset0  # Super Bear
            elif risk == 0.0:
                ww2 = wwmat[:, 0]
            elif risk == 2.0:
                ww2 = wwmat[:, 1]
        ...
        if bsk > maxprice:
            maxprice = bsk
        if maxprice - bsk > maxdd * maxprice:
            maxdd = 1.0 - bsk / maxprice
        # if  bsk - maxprice > maxdd2 :   maxdd2=   bsk - maxprice
        # avgdd+= 1.0 - bsk/maxprice

    penalty = 0.0
    if bsk < criteria[1]:
        penalty = 500.0 * (criteria[1] - bsk) ** 2  # Minimum return
    if var1 > criteria[2]:
    ...
    if crit == 0.0:
        ...
        obj1 = -(bsk - 100) ** 2 + penalty
    elif crit == 2.0:
        obj1 = -(bsk - 100) / var1 + penalty
    elif crit == 3.0:
        obj1 = -(bsk - 100) / math.sqrt(var1) + penalty
    elif crit == 4.0:
        obj1 = -(bsk - 100) ** 2 / (maxdd * maxdd * var1) + penalty
    elif crit == 5.0:
        obj1 = -(bsk - 100) ** 2 / (maxdd * math.sqrt(var1)) + penalty
    elif crit == 6.0:
        obj1 = -(bsk - 100) ** 2 / (avgdd * avgdd) + penalty
    elif crit == 7.0:
        obj1 = -(bsk - 100) / (maxdd * math.sqrt(var1)) + penalty
    elif crit == 8.0:
        obj1 = -(bsk - 100) ** 2 / var1 - (bsk - 100) ** 2 / (maxdd * math.sqrt(var1)) + 2 * penalty
...
class folioOptimizationF(object):
    ...
    """
        ...
        try:
            ...
            self.date_31dec = self.wwbasket["schedule1"]
            self.tlagsignal = self.wwbasket["tlagsignal"]
            self.wwpenalty = self.wwbasket["wwpenalty"]
        except:
            pass
        ...
        # Data check
        # if self.nbrange < 2 : print('ERROR initperiod=2 days')
        if len(self.boundsasset) != self.masset * self.nbregime:
            print(("error Bounds Size", len(self.boundsasset)))

        if self.wwtype == "regimeExtra":
            self.boundsoptim = self.boundsrisk
        elif self.wwtype == "regime":
            self.boundsoptim = self.boundsasset
    ...
    def calcbasket_obj2(self, wwvec):
        ...
        if concenfactor > concenfactor2 or concenfactor < concenfactor - 3 * masset * nbregime:
            return concenfactor * 2.0 - 1000.0  # 4.2*masset*nbregime
        else:
            concenfactor = round(concenfactor, 0) * 2.0
        ...
        crit = np.array(
            [self.wwbasket["opticriteria"], self.wwbasket["minperf"], self.wwbasket["maxvol"]]
        )

        obj1 = calcbasket_objext(
            self.ret,
            self.tmax,
            self.riskind_input,
            wwmat,
            np.array(self.wwasset0),
            self.ww0,
            float(self.nbrange),
            crit,
        )
    ...
    def calcbasket_obj(self, wwvec):
        ...
        bsk = np.zeros(TMAX + 1, dtype=np.float16)
        bsk[0] = 100.0
        ww2 = self.ww0
        lossww = 0.0
        for t in range(1, TMAX + 1):
            ...
            if np.mod(t, rebfreq) == 0:  # rebalancing of weights
                ...
                if costbp != 0:
                    hedgecost = np.sum(np.abs(ww2 - ww2i)) * bsk[t - 1] * costbp  # cost 0.002
                ww2 = ww2i
        ...
        if obj1 < -1000000.0:
            ...
            return obj1 + lossww + concenfactor
...
def folio_concenfactor(ww, masset=12, isnorm=1):
    ...
    if isnorm:
        wwraw = wwraw / np.sum(wwraw, axis=0)
    ss = 0.0
    lss = []
    for k in range(0, 3):
...
def folio_metric(pf4, wwbest, noprint=1, masset=12):
    ...
    vol6mth = (
        volhisto_fromprice(bsk, tmax - 1, 120, axis=0)
        if tyear > 0.5
        else volhisto_fromprice(bsk, tmax - 1, tmax - 1, axis=0)
    )
...
def folio_analysis(date1, bsk, tablefmt="simple"):
    tmax = len(date1)
    if len(bsk) != tmax:
        print("Date and Data not aligned")
        return -1
    ...
    date_31dec = date_getspecificdate(date1, "yearend")
    nyears = len(date_31dec)

    data2 = []
    for i in range(1, nyears):
        bsk1y = bsk[(date_31dec[i - 1]) : (date_31dec[i] + 1)]
        tmax1y = len(bsk1y)
        ...
        data2.append(
            [yearend, rr(bsk[date_31dec[i]]), rr(100 * perf), rr(100 * vol252), rr(sharpe)]
        )

    headers1 = ["YearEnd", "Bsk", "Perf", "Vol1Y", "Sharpe"]
    print(tabulate(data2, headers=headers1, tablefmt=tablefmt))
    print("\n")
  ...
  """
    ...
    for i in range(0, len(draw_pct)):
        if ddstart[i] != ddstart[i - 1] and ddstart[i - 1] != 0 and ddurlast[i - 1] > 10:
            data3.append(
                [
                    datetime_tostring(date1[int(ddstart[i - 1])]),
                    datetime_tostring(date1[int(ddmindate[i - 1])]),
                    rr(draw_pct[ddmindate[i - 1]]),
                    ddurmin[i - 1],
                    ddurlast[i - 1],
                ]
            )
    ...
    print(tabulate(data3, headers=headers1, tablefmt=tablefmt))
    print("\n\n")
    ...
    for i in range(0, len(ddstart)):
        if ddstart[i] != ddstart[i - 1] and ddstart[i - 1] != 0 and ddurlast[i - 1] > 5:
            data3.append(
                [datetime_tostring(date1[int(ddstart[i - 1])]), rr(ddup[i - 1]), ddurlast[i - 1]]
            )
...
def calc_print_correlrank(
    ...
    for kk, i in enumerate(range(0, toprank, 1)):
        stki = int(rank[i, 0])
        namei = symjp1[stki][2:6]
        print(
            kk,
            stki,
            ";" + a_nk400name[util.np_find(namei, a_nk400list)] + ";",
            namei + ";",
            rank[i, 1],
        )
    ...
    # Show Graph--------------------------------------------
    if customnameid2 == []:
        stk_select = util.np_mergelist(
            np.array(rank[nrank - toprank2 : nrank, 0], dtype=int), [refindex]
        )
    else:
        stk_select = util.np_mergelist([refindex], customnameid2)
...
def np_similarity(x, y, wwerr=None, type1=0):
    ...
    if type1 == 1:
        return sci.spatial.distance.correlation(x, y)
    if type1 == 0:
...
class searchSimilarity:
    def __init__(
        ...
        self.dbstartdate = startdate
        self.dbenddate = enddate
    ...
    def load_quotes_fromdb(self, picklefile=""):
        ...
        if picklefile != "":
            ...
            quotes = imp_txt_getquotes(
                self.sym01, self.filejpstock, startdate=self.dbstartdate, endate=self.dbenddate
            )
            self.open1, self.dateref = date_align(
                quotes, type1=self.pricetype
            )  # Get the data and align dates
            del quotes
            util.a_cleanmemory()
    ...
    def __overweight__(self, px):
        ...
        vvmax = util.np_findlocalmax(px)
        xprev = 0
        # print vvmax
        if type(vvmax) != int:
            for x in vvmax:
                if x[0] != 0.0 and np.abs(xprev - x[0]) > 6:  # Max not too close to each other
                    wwerr[max(0, x[0] - 1) : min(nx, (x[0] + 1))] = 1.5
                    xprev = x[0]

        vvmin = util.np_findlocalmin(px)
        xprev = 0
        if type(vvmin) != int:
            for x in vvmin:
    ...
    def set_searchcriteria(
        self,
        name1="7203",
        date1=20160301,
        date2=20160601,
        nlag=1,
        searchperiodstart=20120101,
        typesearch="pattern2",
    ):
        # --------------------  Search Asset Input     -----------------------------------
        ...
        if tstartx == -1:
            print(("Cannot Find Search Start Date:" + str(date1)))
        ...
        if typesearch == "pattern0":
            self.typesearch = 0
        if typesearch == "pattern1":
            self.typesearch = 1
        if typesearch == "pattern2":
            self.typesearch = 2

        self.tstartsrch = tstartsrch
        self.tstartx = tstartx
        self.stockx = stockx
        self.px = px
        self.nx = nx
    ...
    def launch_search(self):
        # --------------------  Search the Pattern    -----------------------------------
        tmaxx = len(self.dateref) - 1
        nstock = len(self.sym01)
        twindow = self.twindow
        tstartx = self.tstartx
        stockx = self.stockx
        px = self.px
        wwerr = self.wwerr
        ...
        for i, x in enumerate(similar_list):
            if similar_list[i, 2] == 0.0:
                similar_list[i, 2] = 1000  # Empty cell
            if similar_list[i, 1] > tstartx - 3:
                similar_list[i, 2] = 100  # Start Element
        similar_list = util.np_sortbycolumn(similar_list, 2, asc=True)
        self.resultrank = similar_list

    def show_comparison_graph(
        self, maxresult=20, show_only_different_time=True, fromid=0, fromend=0, filenameout=""
    ):
        ...
        for i in range(fromid, maxresult):
            ...
            if show_only_different_time and tstarti > self.tstartx - 15:
                pass
                print("Pass same month date")

            else:
                py = price_normalize_1d(
                    self.ret_open1[stocki, tstarti : (tstarti + self.twindow - 1)]
                )
                ...
                plot_price(
                    self.px - 100,
                    py - 100,
                    label=("Similar Stock: " + tit, "Weeks", "% Var"),
                    savename1=filenameout,
                    tickperday=5,
                )
    ...
    def get_rankresult(self, filetosave=""):
        ...
        for i in range(0, 20):
            print(i, vv2[i, 0], vv2[i, 1], "From: " + str(vv2[i, 2]))
...
def imp_a_close_dateref(sym01, sdate=20100101, edate=20160628, datasource="", typeprice="close"):
    ...
    if datasource == "":  # Yahoo / Google
        ...
        stk0 = liststockfile[0]
        # stk2= liststockfile[2]
        liststockname = []
        for k in range(0, len(stk0)):
            liststockname.append((stk0[k])[0:4])
    ...
    for i, q in enumerate(quotes):
        print(
            (str(i) + "_" + sym01[i] + "_" + str(q.date.values[0]) + "_" + str(q.date.values[-1]))
        )
...
def imp_quandl_quotes(symbols, start="20150101", end="20160101", source="google", type1=1):
    ...
    for i, symbol in enumerate(symbols):
        ...
        try:
            if source == "yahoo2":
                ...
                df = None
                if source == "google":
                    df = qd.get("GOOG/" + symbol, trim_start=dd1, authtoken="dffTSNJ4JHbRE1Csd7zZ")
                if source == "yahoo":
                    df = qd.get("YAHOO/" + symbol, trim_start=dd1, authtoken="dffTSNJ4JHbRE1Csd7zZ")
    ...
    # print(errorlist)
    if type1 == 1:
        return quotes, correctlist
    else:
        return quotes, correctlist, errorlist
...
def imp_googleQuote(
    symbol, start_date="20160101", end_date="20160101"
...
def imp_csv_dbupdate(
    ...
    for file1 in filelist:
        ...
        if os.path.isfile(outfile):
            df0 = imp_csv_toext(
                file1=outfile, fromzone="Japan", tozone="UTC", header=0, cols=refcols
            )
        else:
            df0 = None
        ...
        if intype == "csv":
            ...
            df1 = imp_csv_toext(
                file1=infile, header=0, cols=refcols, fromzone="Japan", tozone="UTC"
            )

        if df1 is not None:
            if df0 is not None:
                df1 = imp_pd_merge(df1[refcols], df0[refcols])
            # return outfile, df1
...
def imp_numpy_close_fromdb(
    ...
    dateref0 = dstore.get_histo(
        ["SPY"], table1="daily_us_etf", start1=str(t0), end1=str(t1), split_df=0
    ).date.values
    ...
    for j in range(0, nbatch):
        ...
        if tmax2 < tmax:
            print("Error tmax2 < tmax, wrong dates")
            break
    ...
    for k in range(0, maxasset):
        if close2[k, tmaxx - 1] == 0.0:
            close2[k, tmaxx - 1] = close2[k, tmaxx - 2]
...
def imp_sql_getquotes(
    stocklist01,
    dbname="sqlite:///aaserialize/store/yahoo.db",
    start1=20150101,
    end1=20160616,
    table1="daily_us_stock",
):
    from data import hist_data_storage as yhh
...
def imp_txt_getquotes(
    stocklist01, filedir="E:/_data/stock/daily/20160610/jp", startdate=20150101, endate=20160616
):
...
def imp_csv_toext(
    ...
    if os.path.getsize(file1) > 500:
        ...
        if type1 == "h5":
            ...
            store.append(dfname, df)
            store.close()


def imp_hdfs_db_updatefromcsv(
    dircsv, filepd=r"E:\_data\stock\intraday_google.h5", fromtimezone="Japan", tozone="UTC"
):
    lfile = util.os_file_listall(dircsv, pattern="*.csv", dirlevel=0)
    for x in lfile:
        name1 = (x[0]).split("_")[0]  # get Ticker name, splitting by _
        if util.str_isfloat(name1):
            name1 = "jp" + name1  # Japanese Stocks case
        file1 = x[2]
        print(name1)
        if os.path.getsize(file1) > 500:
...
def imp_hdfs_mergedb(filepdfrom, filepdto):
    ...
    for symbol in list(store0.keys()):
...
def imp_hdfs_storecopy(hdfs1="F:/usstock8.h5", hdfs2="F:/usstock8.h5"):
    store = pd.HDFStore(hdfs1)
    store2 = pd.HDFStore(hdfs2)
    for symbol in list(store.keys()):
...
def imp_hdfs_getquote(filenameh5, dfname="data"):
    try:
        return pd.read_hdf(filenameh5, dfname)
    except:
        return -1.0


def imp_hdfs_getListquote(
    symbols,
    close1="close",
    start="12/18/2015 00:00:00+00:00",
    end="3/1/2016 00:00:00+00:00",
    freq="0d0h10min",
...
def imp_pd_checkquote(quotes):
    for c in quotes:
        print(np.shape(c), c["date"].values[0], c["date"].values[-1])
...
def imp_pd_getclose(df, datefilter=None):
    ...
    df = df.sort("date")
    close = df.interpolate()
    close = close.fillna(method="backfill")  # Interpolate
...
def ta_mar(close, t, m):
    return 100 * (np.mean(close[(t - m) : (t + 1)], axis=0) / close[t])


def ta_ma(close, t, m):
    return np.mean(close[(t - m) : (t + 1)], axis=0)
...
def calc_statestock(close2, dateref, symfull):
    ...
    def perf(close, t0, t1):
        return 100 * (close[:, t1] / close[:, t0] - 1)

    def and2(tuple1):
        return np.logical_and.reduce(tuple1)

    def mar(close, t, m):
        return 100 * (np.mean(close[:, (t - m) : (t + 1)], axis=1) / close[:, t])

    def ma(close, t, m):
        return np.mean(close[:, (t - m) : (t + 1)], axis=1)
    ...
    def dd(t):
        ...
        if x == -1:
            print("Error date not found in dateref " + str(t))
            return np.nan
        else:
            return x
    ...
    stat[k : (k + m), 122] = (
        100 * ma(close2, tmax, 5) / ma(close2, tmax, 50)
    )  # OverSold /OverBought
    ...
    # -------------- Data Fundamental    -------------------------------------------
    df = util.sql_query(
        sqlr="SELECT ticker,shortratio,sector1_id, sector2_id, marketcap   FROM stockfundamental ",
        dburl="/aaserialize/store/finviz.db",
    )
    npdf = df.values
    del df
    npdf0 = npdf[:, 0]
    ...
    for i in range(0, m):
        ...
        stat[i, 50] = kmin + (tmax - 120)
        stat[i, 51] = 100 * pp0 / pmin

        kmax, pmax = util.np_find_maxpos(close2[i, (tmax - 120) : tmax])
        stat[i, 52] = kmax + (tmax - 120)
        stat[i, 53] = 100 * pp0 / pmax
...
#Regression from 2 months ago
 ...
 """
    ...
    for i in range(0, m):
        stat[i, 66] = np_countretsign(close2[i, (tmax - 5) : tmax])

    # Nb of Positive days last 10d
    for i in range(0, m):
        stat[i, 67] = np_countretsign(close2[i, (tmax - 10) : tmax])
...
def imp_finviz():
    ...
    while currentpage > 0:
        ...
        secondurl = (
            "http://www.finviz.com/screener.ashx?v="
            + str(111)
            + "&f=geo_usa"
            + "&r="
            + str(currentpage)
