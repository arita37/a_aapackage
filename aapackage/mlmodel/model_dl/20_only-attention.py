...
class Model:
    def __init__(
        self, seq_len, learning_rate, dimension_input, dimension_output,epoch=100
    ):
        self.epoch = epoch
...
def fit(model, data_frame):
    ...
    for i in range(model.epoch):
        ...
        for k in range(0, (data_frame.shape[0] // model.timestep) * model.timestep, model.timestep):
            ...
            loss = np.mean(loss)
            total_loss += loss
        total_loss /= df_log.shape[0] // model.timestep
        if (i + 1) % 100 == 0:
...
def test(filename= 'dataset/GOOG-year.csv') :
    ...
    module, model =create('20_only-attention.py',{"epoch": 1, "seq_len": 5.0, "learning_rate": 0.01, "dimension_input": df_log.shape[1], "dimension_output": df_log.shape[1]})
