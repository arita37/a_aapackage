...
class Model:
    def __init__(
        ...
        query = tf.layers.dense(
            tf.expand_dims(last_state[:, size_layer:], 1), attention_size
        )
        ...
        align_dec = tf.reduce_sum(
            attention_w * tf.tanh(keys_dec + query_dec), [2]
        )
        align_dec = tf.nn.tanh(align_dec)
        self.outputs_dec = tf.squeeze(
            tf.matmul(
                tf.transpose(self.outputs_dec, [0, 2, 1]),
                tf.expand_dims(align_dec, 2),
            ),
            2,
        )
...
def fit(model,data_frame):
    ...
    for i in range(model.epoch):
        init_value = np.zeros((model.timestep, model.hidden_layer_size))
        total_loss = 0
        for k in range(0, (data_frame.shape[0] // model.timestep) * model.timestep, model.timestep):
            ...
            batch_x = np.expand_dims(data_frame.iloc[k:index].values, axis = 1)
    ...
    if upper_b == model.timestep:
        ...
        for k in range(0, (data_frame.shape[0] // model.timestep) * model.timestep, model.timestep):
            out_logits, last_state = sess.run(
                ...
                feed_dict = {
                    model.X: np.expand_dims(
                        data_frame.iloc[k : k + model.timestep].values, axis = 1
                    ),
                    model.hidden_layer: init_value,
                },
            )
...
def test(filename= 'dataset/GOOG-year.csv') :
    ...
    module, model =create('15_lstm-seq2seq-attention.py',{"epoch": 1, "timestep": 5.0, "learning_rate": 0.01, "num_layers": 1.0, "size": df_log.shape[1], "size_layer": 128.0, "output_size": df_log.shape[1]})
