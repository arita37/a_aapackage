...
def fit(model, data_frame):
    ...
    for i in range(model.epoch):
        ...
        for k in range(0, data_frame.shape[0] - 1, model.timestep):
            ...
            loss = np.mean(
                loss
            )  # dont know why you are taking the average of the loss more than one time
...
if __name__ == "__main__":
    ...
    for i in range(future_day - 1):
        out_logits, init_value = predict(
            modelnn, sess, df_log.iloc[-timestamp:, :], True, init_value
        )
