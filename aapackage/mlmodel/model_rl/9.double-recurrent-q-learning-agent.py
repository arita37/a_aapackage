...
class Agent:
    ...
    def __init__(self, state_size, window_size, trend, skip):
        ...
        self.model = Model(
            self.state_size, self.OUTPUT_SIZE, self.LAYER_SIZE, self.LEARNING_RATE, "real_model"
        )
        self.model_negative = Model(
            self.state_size, self.OUTPUT_SIZE, self.LAYER_SIZE, self.LEARNING_RATE, "negative_model"
        )
    ...
    def _construct_memories(self, replay):
        ...
        Q = self.sess.run(
            self.model.logits,
            feed_dict={self.model.X: states, self.model.hidden_layer: init_values},
        )
        Q_new = self.sess.run(
            self.model.logits,
            feed_dict={self.model.X: new_states, self.model.hidden_layer: init_values},
        )
        Q_new_negative = self.sess.run(
            self.model_negative.logits,
            feed_dict={
    ...
    def buy(self, initial_money):
        ...
        for t in range(0, len(self.trend) - 1, self.skip):
            action, last_state = self.sess.run(
                [self.model.logits, self.model.last_state],
                feed_dict={
                    self.model.X: [self.INITIAL_FEATURES],
        ...
        for i in range(iterations):
            ...
            for t in range(0, len(self.trend) - 1, self.skip):
                ...
                cost, _ = self.sess.run(
                    ...
                    feed_dict={self.model.X: X, self.model.Y: Y, self.model.hidden_layer: INIT_VAL},
                )
                self.T_COPY += 1
