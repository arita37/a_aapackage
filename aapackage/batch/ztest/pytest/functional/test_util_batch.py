...
def test_os_folder_rename():
    # Test : correct args
    old = appendToAssetsPath("mockfolder3_rename")
    new = appendToAssetsPath("mockfolder3_renamed")
    ...
    #Cleanup
    os.rename(appendToAssetsPath(folders[0]),appendToAssetsPath("mockfolder4_rename"))

def test_os_folder_create():
    # Test : correct args
    folder = appendToAssetsPath("testFolder")
...
def test_batch_run_infolder():
    ...
    valid_folders = [appendToAssetsPath("mockfolder1_util_batch")]
    sub_process_list = util_batch.batch_run_infolder(valid_folders,waitsleep=0)

    # Cleaning up
    src = appendToAssetsPath("mockfolder1_util_batch_qstart")
    dst = appendToAssetsPath("mockfolder1_util_batch")
    ...
    valid_folders = [appendToAssetsPath("mockfolder2_util_batch")]
    with pytest.raises(OSError):
        util_batch.batch_run_infolder(valid_folders, suffix="_suffix_test", waitsleep=0)
    """

    # Test : folder doesn't exist
    valid_folders = [appendToAssetsPath("mockfolder_doesnt_exist")]
    with pytest.raises(OSError):
    ...
    valid_folders = [appendToAssetsPath("mockfolder2_util_batch_suffix_test")]
    with pytest.raises(OSError):
        util_batch.batch_run_infolder(valid_folders, waitsleep=0)
    """
...
def test_batch_parallel_subprocess():
    ...
    with pytest.raises(OSError):
        csv = appendToAssetsPath("file_doesnt_exist.csv")
        script = appendToAssetsPath("mockscript1.py")
    ...
    # doesn't raise exception because python is directly called , not the script file 
    with pytest.raises(OSError):
        csv = appendToAssetsPath("mockfile.csv")
        script = appendToAssetsPath("script_doesnt_exist.py")
        util_batch.batch_parallel_subprocess(csv,script,waitime=0)
    """
