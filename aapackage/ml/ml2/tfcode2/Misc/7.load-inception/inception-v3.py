...
get_ipython().run_cell_magic('time', '', 'sess.run(logits,feed_dict={X:img})\n# first time slow, GPU caching')


# In[14]:


get_ipython().run_cell_magic('time', '', 'labels[str(np.argmax(sess.run(logits,feed_dict={X:image})[0]))]\n# repeat same experiment to get accurate time')
