...
class Tacotron:
    def __init__(self, is_training=True, reuse=None):
        ...
        lookup_table = tf.concat((tf.zeros(shape=[1, embed_size]), lookup_table[1:, :]), 0)
        ...
        with tf.variable_scope("encoder", reuse=reuse):
            ...
            enc = conv1d_banks(prenet_out_encoder, K=decoder_num_banks, is_training=is_training)
            enc = tf.layers.max_pooling1d(enc, pool_size=2, strides=1, padding="same")
            enc = tf.layers.conv1d(enc, embed_size // 2, 3, name="encoder-conv1-1", padding="SAME")
            enc = tf.nn.relu(tf.layers.batch_normalization(enc, training=is_training))
            enc = tf.layers.conv1d(enc, embed_size // 2, 3, name="encoder-conv1-2", padding="SAME")
            enc = tf.layers.batch_normalization(enc, training=is_training)
            enc += prenet_out_encoder
            for i in range(num_highwaynet_blocks):
                enc = highwaynet(
                    enc, num_units=embed_size // 2, scope="encoder-highwaynet-{}".format(i)
                )
            with tf.variable_scope("encoder-gru", reuse=reuse):
                ...
                outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell, cell_bw, enc, dtype=tf.float32)
                self.memory = tf.concat(outputs, 2)
        with tf.variable_scope("decoder-1", reuse=reuse):
            prenet_out_decoder1 = prenet(self.decoder_inputs, is_training=is_training)
            with tf.variable_scope("attention-decoder-1", reuse=reuse):
                attention_mechanism = tf.contrib.seq2seq.BahdanauAttention(embed_size, self.memory)
                decoder_cell = tf.contrib.rnn.GRUCell(embed_size)
                cell_with_attention = tf.contrib.seq2seq.AttentionWrapper(
                    decoder_cell, attention_mechanism, embed_size, alignment_history=True
                )
                outputs_attention, state_attention = tf.nn.dynamic_rnn(
                    cell_with_attention, prenet_out_decoder1, dtype=tf.float32
                )
            self.alignments = tf.transpose(state_attention.alignment_history.stack(), [1, 2, 0])
            with tf.variable_scope("decoder1-gru-1"):
                cell = tf.contrib.rnn.GRUCell(embed_size)
                outputs, _ = tf.nn.dynamic_rnn(cell, outputs_attention, dtype=tf.float32)
        ...
        with tf.variable_scope("decoder-2", reuse=reuse):
            ...
            dec = conv1d_banks(out_decoder2, K=decoder_num_banks, is_training=is_training)
            ...
            dec = tf.nn.relu(tf.layers.batch_normalization(dec, training=is_training))
