...
class Stemmer:
    def __init__(
        ...
        decoder_cell = attention(
            self.encoder_out, self.dnc_cell_dec, self.X_seq_len, self.encoder_state
        )
...
for i in range(epoch):
    ...
    for k in range(0, (len(test_X) // batch_size) * batch_size, batch_size):
        ...
        predicted, loss = sess.run(
            [model.predicting_ids, model.cost],
            feed_dict={
                model.X: batch_x,
                model.Y: batch_y,
                model.X_seq_len: seq_x,
    ...
    print(
        "epoch: %d, avg valid loss: %f, avg valid accuracy: %f\n"
        % (i + 1, total_loss_validation, total_accuracy_validation)
    )
...
predicted, loss = sess.run(
    [model.predicting_ids, model.cost],
    feed_dict={model.X: batch_x, model.Y: batch_y, model.X_seq_len: seq_x, model.Y_seq_len: seq_y},
)
