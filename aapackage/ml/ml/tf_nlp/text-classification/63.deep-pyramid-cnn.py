...
class Model:
    def __init__(
        ...
        first_region = tf.layers.conv1d(
            embedded, num_filters, kernel_size=kernel_size, strides=1, padding="valid"
        )
        forward = tf.nn.relu(first_region)
        forward = tf.layers.conv1d(
            forward, num_filters, kernel_size=kernel_size, strides=1, padding="same"
        )
        ...
        def _block(x):
            ...
            x = tf.nn.relu(px)
            x = tf.layers.conv1d(x, num_filters, kernel_size=kernel_size, strides=1, padding="same")
        ...
        self.logits = tf.reduce_sum(
            tf.layers.conv1d(forward, dimension_output, kernel_size=1, strides=1, padding="SAME"), 1
        )
...
model = Model(
    maxlen, dimension_output, len(dictionary), embedding_size, kernel_size, num_filters, 1e-3
)
