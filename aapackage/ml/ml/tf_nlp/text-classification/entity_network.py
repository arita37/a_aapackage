...
class EntityNetwork:
    def __init__(
        ...
        self.story = tf.placeholder(
            tf.int32, [None, self.story_length, self.sequence_length], name="story"
        )
    ...
    def output_module(self):
        p = tf.nn.softmax(
            tf.multiply(tf.expand_dims(self.query_embedding, axis=1), self.hidden_state)
        )
    ...
    def rnn_story(self):
        ...
        h_all = tf.get_variable(
            "hidden_states", shape=[self.block_size, self.dimension], initializer=self.initializer
        )
        w_all = tf.get_variable(
            "keys", shape=[self.block_size, self.dimension], initializer=self.initializer
        )
    ...
    def embedding_with_mask(self):
        ...
        story_mask = tf.get_variable(
            "story_mask", [self.sequence_length, 1], initializer=tf.constant_initializer(1.0)
        )
        query_mask = tf.get_variable(
            "query_mask", [self.sequence_length, 1], initializer=tf.constant_initializer(1.0)
        )
    ...
    def input_encoder_bi_lstm(self):
        ...
        query_hidden_output, _ = tf.nn.bidirectional_dynamic_rnn(
            lstm_fw_cell, lstm_bw_cell, self.query_embedding, dtype=tf.float32, scope="query_rnn"
        )
        ...
        self.story_embedding = tf.reshape(
            ...
            shape=(-1, self.story_length * self.sequence_length, self.embed_size),
        )
    ...
    def instantiate_weights(self):
        ...
        with tf.variable_scope("output_module"):
            self.H = tf.get_variable(
                "H", shape=[self.dimension, self.dimension], initializer=self.initializer
            )
            self.R = tf.get_variable(
                "R", shape=[self.dimension, self.num_classes], initializer=self.initializer
            )
        ...
        with tf.variable_scope("dynamic_memory"):
            self.U = tf.get_variable(
                "U", shape=[self.dimension, self.dimension], initializer=self.initializer
            )
            self.V = tf.get_variable(
                "V", shape=[self.dimension, self.dimension], initializer=self.initializer
            )
            self.W = tf.get_variable(
                "W", shape=[self.dimension, self.dimension], initializer=self.initializer
            )
    ...
    def cell(self, s_t, h_all, w_all, i):
        ...
        h_candidate_part1 = tf.reshape(
            h_candidate_part1, shape=(self.batch_size, self.block_size, self.dimension)
        )
        h_candidate_part2 = tf.expand_dims(tf.matmul(s_t, self.W) + self.h2_bias, axis=1)
        h_candidate = self.activation(
            h_candidate_part1 + h_candidate_part2, scope="h_candidate" + str(i)
        )
