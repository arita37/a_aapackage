...
class DynamicMemoryNetwork:
    def __init__(
        ...
        self.story = tf.placeholder(tf.int32, [None, self.story_length, self.sequence_length])
        ...
        correct_prediction = tf.equal(tf.cast(self.predictions, tf.int32), self.answer_single)
    ...
    def input_module(self):
        ...
        story_embedding = tf.reshape(
            story_embedding,
            (self.batch_size, self.story_length, self.sequence_length * self.embed_size),
        )
    ...
    def episodic_memory_module(self):  # input(story):[batch_size,story_length,hidden_size]
        ...
        """
        candidate_inputs = tf.split(self.story_embedding, self.story_length, axis=1)
        ...
        for pass_number in range(self.num_pass):
            g = self.attention_mechanism_parallel(
                self.story_embedding, m_current, self.query_embedding, pass_number
            )
    ...
    def gated_gru(self, c_current, h_previous, g_current):
        ...
        """
        h_candidate = self.gru_cell(c_current, h_previous, "gru_candidate_sentence")
        h_current = tf.multiply(g_current, h_candidate) + tf.multiply(1 - g_current, h_previous)
        return h_current
    ...
    def x1Wx2_parallel(self, x1, x2, scope):
        ...
        with tf.variable_scope(scope):
            ...
            x1_w = tf.layers.dense(x1, self.story_length * self.hidden_size, use_bias=False)
            x1_w_expand = tf.expand_dims(x1_w, axis=2)
            x1_w_x2 = tf.matmul(x1_w_expand, x2)
            x1_w_x2 = tf.reshape(
                x1_w_x2,
                shape=(self.batch_size, self.story_length, self.hidden_size, self.hidden_size),
            )
    ...
    def gru_cell(self, Xt, h_t_minus_1, variable_scope):
        ...
        with tf.variable_scope(variable_scope):
            ...
            h_t_candiate = tf.nn.tanh(
                tf.matmul(Xt, self.W_h) + r_t * (tf.matmul(h_t_minus_1, self.U_h)) + self.b_h
            )
    ...
    def instantiate_weights(self):
        ...
        with tf.variable_scope("gru_cell"):
            self.W_z = tf.get_variable(
                "W_z", shape=[self.embed_size, self.hidden_size], initializer=self.initializer
            )
            self.U_z = tf.get_variable(
                "U_z", shape=[self.embed_size, self.hidden_size], initializer=self.initializer
            )
            self.b_z = tf.get_variable("b_z", shape=[self.hidden_size])
            # GRU parameters:reset gate related
            self.W_r = tf.get_variable(
                "W_r", shape=[self.embed_size, self.hidden_size], initializer=self.initializer
            )
            self.U_r = tf.get_variable(
                "U_r", shape=[self.embed_size, self.hidden_size], initializer=self.initializer
            )
            self.b_r = tf.get_variable("b_r", shape=[self.hidden_size])

            self.W_h = tf.get_variable(
                "W_h", shape=[self.embed_size, self.hidden_size], initializer=self.initializer
            )
            self.U_h = tf.get_variable(
                "U_h", shape=[self.embed_size, self.hidden_size], initializer=self.initializer
            )
