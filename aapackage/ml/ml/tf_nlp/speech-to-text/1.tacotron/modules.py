...
def normalize_bn(inputs, decay=0.99, is_training=True, activation_fn=None, scope="normalize_bn"):
...
def normalize_layer_norm(inputs, activation_fn=None, scope="normalize_layer_norm"):
    return tf.contrib.layers.layer_norm(
        inputs=inputs, center=True, scale=True, activation_fn=activation_fn, scope=scope
    )
...
def attention_decoder(inputs, memory, units=None, scope="attention_decoder", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        if units is None:
            units = inputs.get_shape().as_list()[-1]
        attention_mechanism = tf.contrib.seq2seq.BahdanauAttention(units, memory)
...
def prenet(inputs, is_training=True, scope="prenet", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        outputs = tf.layers.dense(inputs, units=embed_size, activation=tf.nn.relu, name="dense1")
        outputs = tf.nn.dropout(
            outputs, keep_prob=0.5 if is_training == True else 1.0, name="dropout1"
        )
        outputs = tf.layers.dense(
            outputs, units=embed_size // 2, activation=tf.nn.relu, name="dense2"
        )
        outputs = tf.nn.dropout(
            outputs, keep_prob=0.5 if is_training == True else 1.0, name="dropout2"
        )
    return outputs
...
def highwaynet(inputs, units=None, scope="highwaynet", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        if units is None:
            units = inputs.get_shape().as_list()[-1]
        H = tf.layers.dense(inputs, units=units, activation=tf.nn.relu, name="dense1")
        T = tf.layers.dense(inputs, units=units, activation=tf.nn.sigmoid, name="dense2")
        C = 1.0 - T
        return H * T + inputs * C
