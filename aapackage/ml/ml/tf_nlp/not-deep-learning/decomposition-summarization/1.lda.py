...
article = [
    ...
    "Do you have information you want to share with HuffPost? Here\xe2\x80\x99s how.",
]
...
article = [
    ...
    if re.match("^[a-zA-Z_-]*$", word) or "." in word or "'" in word or '"' in word
]
...
lda = LatentDirichletAllocation(n_components=len(sentences)).fit_transform(bow)
...
summary = [
    (sentences[i], np.linalg.norm(np.dot(lda, bow.todense()), 2))
    for i in range(len(sentences))
    for b in range(len(lda))
]
...
# get top 10 of sentences
k = 10

".".join([a for a, b in summary][len(summary) - (k) :])
