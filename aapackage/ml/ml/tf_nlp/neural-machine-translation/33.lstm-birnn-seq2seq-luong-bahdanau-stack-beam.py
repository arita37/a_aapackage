    ...
    def __init__(
        ...
        def bahdanau(size):
            attention_mechanism = tf.contrib.seq2seq.BahdanauAttention(
                num_units=size, memory=self.encoder_out
            )
            return tf.contrib.seq2seq.AttentionWrapper(
                cell=cells(size), attention_mechanism=attention_mechanism, attention_layer_size=size
            )
        ...
        with tf.variable_scope("decode"):
            attention_mechanism = tf.contrib.seq2seq.LuongAttention(
                num_units=size_layer, memory=self.encoder_out, memory_sequence_length=self.X_seq_len
            )
        ...
        with tf.variable_scope("decode", reuse=True):
            ...
            luong_cells = tf.contrib.seq2seq.AttentionWrapper(
                cell=tf.nn.rnn_cell.MultiRNNCell(
                    [cells(size_layer, reuse=True) for _ in range(num_layers)]
                ),
                attention_mechanism=attention_mechanism,
                attention_layer_size=size_layer,
            )
