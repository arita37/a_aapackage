...
class Model:
    def __init__(self, dimension_picture, learning_rate, dimension_output):
        ...
        self.cost_1 = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(self.y_hat_1, self.Y_1)
        )
        self.cost_2 = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(self.y_hat_2, self.Y_2)
        )
