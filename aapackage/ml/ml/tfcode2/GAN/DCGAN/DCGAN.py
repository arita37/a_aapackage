...
for i in xrange(EPOCH):
    ...
    print("Epoch {}/{}...".format(i + 1, EPOCH), "Discriminator Loss: {:.4f}".format(loss_D), "Generator Loss: {:.4f}".format(loss_G))
...
for i in xrange(EPOCH):
    ...
    for x in xrange(0, trainset.shape[0] - BATCH_SIZE, BATCH_SIZE):
        batch_z = np.random.uniform(-1, 1, size = (BATCH_SIZE, 100))
        _, lossg1 = sess.run([model.g_train_opt, model.g_loss], feed_dict = {model.Z: batch_z, model.inputs: second_scale(trainset[x: x + BATCH_SIZE, :, :, :])})
        _, lossg2 = sess.run([model.g_train_opt, model.g_loss], feed_dict = {model.Z: batch_z, model.inputs: second_scale(trainset[x: x + BATCH_SIZE, :, :, :])})
        _, lossd = sess.run([model.d_train_opt, model.d_loss], feed_dict = {model.inputs: second_scale(trainset[x: x + BATCH_SIZE, :, :, :]), model.Z: batch_z})
        
        loss_D += lossd; loss_G += ((lossg1 + lossg2) / 2)
