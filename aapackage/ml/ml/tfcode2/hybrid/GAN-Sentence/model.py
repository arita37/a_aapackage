...
def generate(sess, sequence, noise, model, tag, length_sentence, text_vocab):
    ...
    for i in xrange(0, (len(tag) // sequence) * sequence, sequence):
        ...
        batch_fake[0, :, :] = tokens
        try:
            prob, last_state = sess.run(
                [model.final_outputs_generator, model.last_state_generator],
                feed_dict={model.initial_layer: last_state, model.fake_input: batch_fake},
            )
        except Exception as e:
            prob, last_state = sess.run(
                [model.final_outputs, model.last_state],
                feed_dict={model.noise: noise, model.fake_input: batch_fake},
            )
...
def generator_encode(X, num_layers, size_layer, len_noise, reuse=False):
    with tf.variable_scope("generator_encode", reuse=reuse):
        rnn_cells = tf.nn.rnn_cell.MultiRNNCell(
            [lstm_cell(size_layer, False) for _ in xrange(num_layers)], state_is_tuple=False
        )
...
def generator_sentence(
    X, hidden_layer, num_layers, size_layer, dimension_input, name_scope, reuse=False
):
    with tf.variable_scope(name_scope, reuse=reuse):
        ...
        outputs, last_state = tf.nn.dynamic_rnn(
            rnn_cells, X, initial_state=hidden_layer, dtype=tf.float32
        )
        ...
        return (
            tf.reshape(tf.nn.softmax(logits), (seq_shape[0], seq_shape[1], dimension_input)),
            last_state,
            logits,
        )
...
class Model:
    def __init__(
        self, num_layers, size_layer, dimension_input, len_noise, sequence_size, learning_rate
    ):
        ...
        self.final_outputs, self.last_state, _ = generator_sentence(
            self.fake_input,
            self.initial_layer,
            num_layers,
            size_layer,
        ...
        true_logits = discriminator(
            self.true_sentence, num_layers, size_layer, dimension_input, reuse=True
        )
        ...
        g_vars = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="generator_encode"
        ) + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="generator_sentence")
        self.d_train_opt = tf.train.GradientDescentOptimizer(learning_rate).minimize(
            self.d_loss, var_list=d_vars
        )
        self.g_train_opt = tf.train.AdamOptimizer(learning_rate).minimize(
            self.g_loss, var_list=g_vars
        )
        self.final_outputs_generator, self.last_state_generator, logits_generator = generator_sentence(
        ...
        self.seq_loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(logits=logits_generator, labels=y_batch_long)
        )
        seq_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="generator_sequence")
        self.seq_opt = tf.train.RMSPropOptimizer(0.01, 0.9).minimize(
            self.seq_loss, var_list=seq_vars
        )
