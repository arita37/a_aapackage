...
class NeuroEvolution:
    ...
    def evolve(self, generations=20, checkpoint=5):
        ...
        for epoch in range(generations):
            ...
            scores = [
                self.evaluate(p.last_features, self.novel_backlog, self.novel_pop)
            ...
            if np.random.randint(0, 10) < novelty_search_threshold:
                ...
                print(
                    "novel add fittest, score: %f, backlog size: %d"
                    % (pop_sorted[0].fitness, len(self.generation_backlog))
                )
