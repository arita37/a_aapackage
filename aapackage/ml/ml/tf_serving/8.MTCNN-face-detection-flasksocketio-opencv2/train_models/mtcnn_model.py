...
def P_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
    ...
    with slim.arg_scope(
        ...
        bbox_pred = slim.conv2d(
            net, num_outputs=4, kernel_size=[1, 1], stride=1, scope="conv4_2", activation_fn=None
        )
        print(bbox_pred.get_shape())
        # batch*H*W*10
        landmark_pred = slim.conv2d(
            net, num_outputs=10, kernel_size=[1, 1], stride=1, scope="conv4_3", activation_fn=None
        )
...
def R_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
    with slim.arg_scope(
        ...
        cls_prob = slim.fully_connected(
            fc1, num_outputs=2, scope="cls_fc", activation_fn=tf.nn.softmax
        )
        ...
        landmark_pred = slim.fully_connected(
            fc1, num_outputs=10, scope="landmark_fc", activation_fn=None
        )
        print(landmark_pred.get_shape())
        # train
        if training:
