...
def push_sentiment(**context):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        output_sentiment = sess_sentiment.run(logits_sentiment, feed_dict={x_sentiment: batch_x})
...
push_sentiment_task = PythonOperator(
    task_id="push_sentiment", python_callable=push_sentiment, provide_context=True, dag=DAG
)
...
pull_to_elastic_task = PythonOperator(
    task_id="pull_to_elastic", python_callable=pull_to_elastic, provide_context=True, dag=DAG
)
