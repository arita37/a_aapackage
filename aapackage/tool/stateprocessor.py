...
def ff(x):
    return util.find(x, symfull)  # 607
...
def process_stock(stkstr, show1=1):
    ...
    for k, x in enumerate(stklist):
        stklist[k] = x.strip().replace("\n", "")
    v = list(set(stklist))
    v.sort()
    aux = ""
    for x in v:
...
def show(ll):
    ...
    for x in ll:
        ...
        if k == -1:
            ...
            print(
                "Ret 20 days:", s1[k, 3], s1[k, 4], s1[k, 5], s1[k, 6], s1[k, 7], s1[k, 8]
            )  # Return
...
def get_treeselect(stk, s1, xnewdata=None, newsample=5, show1=1, nbtree=5, depthtree=10, debug=0):
    ...
    errmin = 20
    diversity = 0
    divermax = newsample  # max(10, np.sum(Ytrain) /3)
...
def cond_buy(v, t):
    pt = v[t]
    if (
        0.02 < v[t + 3] / pt - 1 < 0.15
        and v[t + 5] / pt - 1 > 0.06
        and v[t + 10] / v[t] - 1.0 > 0.07
    ):
        return True
...
def cond_buy2(v, t):
    pt = v[t]
    if (
        0.02 < v[t + 3] / pt - 1 < 0.06
        and v[t + 5] / pt - 1 > 0.04
        and v[t + 10] / v[t] - 1.0 > 0.05
    ):
        return True
...
def cond_buy3(v, t):
    pt = v[t]
    if (
        0.02 < v[t + 3] / pt - 1 < 0.08
        and v[t + 5] / pt - 1 > 0.03
        and 0.04 < v[t + 7] / v[t] - 1.0 < 0.20
    ):
        return True
    else:
        return False
...
def selection_stock(close2, criteria1):
    ...
    for t in range(0, tmax - 10):
        for k in range(0, masset):
            if criteria1(close2[k, :], t):
                stk_select.append([t, k])
