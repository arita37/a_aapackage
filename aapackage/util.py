...
def session_save(filename="/folder1/name1", globals1=None):
   ...
   BIG issue with Import, Impor FULL MODULE ----> BIG ISSUE
    ...
    data = spyutil.globalsfilter(
        globals1,
        check_all=True,
        filters=tuple(spybas.get_supported_types()["picklable"]),
...
def pd_toexcel_many(
    ...
    if df2 is not None:
        pd_toexcel(df2, outfile, sheet_name="df2")
    if df3 is not None:
        pd_toexcel(df3, outfile, sheet_name="df3")
    if df4 is not None:
        pd_toexcel(df4, outfile, sheet_name="df4")
    if df5 is not None:
        pd_toexcel(df5, outfile, sheet_name="df5")
    if df6 is not None:
...
def plot_heatmap(frame, ax=None, cmap=None, vmin=None, vmax=None, interpolation="nearest"):
    ...
    ax.set_aspect("auto")
    ax.imshow(
        frame.values, interpolation=interpolation, cmap=cmap, vmin=vmin, vmax=vmax, aspect="auto"
    )
    return
        ...
        if field["name"] in source_dict:
            ...
            if field["type"].lower() == "record" and "fields" in field:
                ...
                # record
                    ...
                    dest_dict[field["name"]] = {}
                    np_map_dict_to_bq_schema(
                        source_dict[field["name"]], field, dest_dict[field["name"]]
                    )
            # list
