...
qlist, sym = pf.imp_sql_getquotes(sym, start1="19950110", end1=now1,
                                  table1='daily_us_etf')  # .imp_yahoo_getquotes(sym, start="19950110", end=now1)
for k, q in enumerate(qlist): print(sym[k], q.date.values[0], q.date.values[-1])
...
# Check of the dates
for k, q in enumerate(qlist): print(sym[k], date0[0], date0[-1])

# Indicator and Index Selection
select_ind = ['MA_200_dist', 'MA_50_dist', 'MA_20_dist', 'MA_5_dist', 'MA_3_dist',
              'RET_1', 'RET_2', 'RET_5', 'RET_20', 'RET_60',
              'RMI_14_10', 'RMI_7_5', 'STD_120', 'ndaylow_252',
              'ndistlow_252', 'ndayhigh_252', 'ndisthigh_252',
              'qearning_per', 'qearning_day', 'optexpiry_day'
              ]
...
for j, qiname in enumerate(select_idx):
    ...
    for k, colkname in enumerate(select_ind):
        ...
        state = util.pd_insertcol(state, qiname.replace('=X', '') + '_' + colkname, qaux[colkname].values)
del qaux, qlist;
util.a_cleanmemory()
...
def and1(x, y): return np.logical_and(x, y)
...
for predlag in [1, 2, 3, 4, 5, 10, 15, 20]:
    for retlevel in [-0.04, -0.02, -0.01, -0.005, 0.0, 0.005, 0.01, 0.02, 0.04]:
        vv = np.copy(
            util.np_sortbycol(smodel2[and1(smodel2[:, 4] == predlag, smodel2[:, 9] == retlevel)], 3, asc=False))
        ...
        if len(vv) > 2:
            ...
            if len(cmref) > 1: print((predlag, retlevel, cm_normref[0, 0] + cm_normref[1, 1])); print(cm_normref); print(
            cmref)
            ...
            if len(cm_norm) > 1 and len(cmref) > 1:
                print((cm[0, 0] + cm[1, 1] - cmref[0, 0] - cmref[1, 1], cm_norm[0, 0] + cm_norm[1, 1]));
                print(cm_norm);
                print((cm), selectk)
                print(selectkbest);
                estwwbest
            ...
            util.save_obj(vv, 'prediction1/predictor_spy_' + str(predlag) + 'day_' + str(retlevel) + '_ret')
...
Ypredict = util.pd_array_todataframe(qspy[['date', 'close', 'MA_3']].values[5282:],
                                     ['date', 'close', 'MA_3'], index1=qspy.index[5282:].values)

predlaglist = [10, 15, 20];
rpct = [-0.04, -0.02, -0.01, 0.0, 0.01, 0.02, 0.04]
for k in predlaglist:
    aux = []
    for i in rpct: aux.append(str(k) + 'd' + str(i))
...
datefut = util.datetime_todate(
    util.date_generatedatetime(start=util.datetime_tostring(util.date_add_bdays(state.date.values[-1], 1)), nbday=90))
...
def addvote(x, k, nmax):
    val = 1
    if k == 0:    val = 10  # Extreme: Strong Predictor
    if k == nmax: val = 7  # Extreme: Strong Predictor  right
    if x == 1:  # Right
        if k == nmax:
...
def findret(r1, kk, rpct):
    if np.isnan(np.sum(r1)): return 0
    nmax = len(rpct);
    vote = np.zeros(nmax + 1)
    for k in range(0, nmax):    vote = vote + addvote(r1[k], k, nmax)
    # print vote
...
for predlag in predlaglist:
    ...
    for retlevel in rpct:
        ...
        tt0 = 200;
        ttest = 5282 - tt0
        vv = util.load_obj('prediction1/predictor_spy_' + str(predlag) + 'day_' + str(retlevel) + '_ret')
...
def median2(x1, x2): return np.mean(qma3_r1[and1(qma3_r1 > x1, qma3_r1 < x2)])
...
Ypredict2 = util.pd_array_todataframe(Ypredict[['date', 'close', 'MA_3']].values, ['date', 'close', 'MA_3'],
...
for predlag in [1, 2, 3, 4, 5, 10, 15, 20]:  # [1,2,3,4,5] :
    for retlevel in [-0.04, -0.02, -0.01, -0.005, 0.0, 0.005, 0.01, 0.02,
                     0.04]:  # [ -0.01, -0.005,  0.0,  0.005,  0.01  ] :
        try:
            tt0 = 200;
            ttest = 5282 - tt0
            vv = util.load_obj('prediction1/predictor_spy_' + str(predlag) + 'day_' + str(retlevel) + '_ret')
            ...
            cm_norm = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis]
            print((
            "Lag:" + str(predlag), 'Ret:' + str(retlevel), round(cm_norm[0, 0] + cm_norm[1, 1], 3), cm[0, 0], cm[0, 1],
            cm[1, 0], cm[1, 1]))
        except:
            pass
#####################################################################################
