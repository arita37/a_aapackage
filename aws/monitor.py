...
def aws_region_get_keypair(ec2, outype=0) :
   ...
   if len(rs) < 1 :
      ...
      if outype== 0:       return x.name
      if outype== 1:       return x.name, x
...
def instance_get_allactive() :
   return instance_spot_get_allactive(accounts=None, region_names=['ap-northeast-2'], filters= {}, isprint=0)
...
def instance_spot_request_getall(accounts=None, region_names=['ap-northeast-2'], filters= {}, isprint=0) :
   """Will find all requests across all EC2 regions for all accounts
      # filters={'instance-state-name': 'running'}
   """
   if not accounts:
      creds= aws_accesskey_get(); accounts={'main': creds}

   if len(region_names) > 0 :  regions= [boto.ec2.get_region(x)  for x in region_names   ]
   else :                      regions= boto.ec2.regions()

   requests={};  rsv_by_account_region=[]
      ...
      for region in regions:
         ...
         if conn is not None :
           ...
           if isprint :
             ...
             for x in rsvlist :  print("_____", x.id,  x.state, x.price, x.instance_id)
...
def instance_spot_get_allactive(accounts=None, region_names=['ap-northeast-2'], filters= {}, isprint=0) :
   ...
   running_instances={};  rsv_by_account_region=[]
   for account_name in accounts:
      ...
      for region in regions:
         ...
         if conn is not None :
           ...
           if isprint :
             print((account_name, region.name))
             for x in rsvlist :  print("_____", x.id,  x.state)
...
def instance_spot_get_pricelast(conn=None, inst_type='C3.Large', region=''):
   price_history=conn.get_spot_price_history(instance_type=inst_type, product_description='Linux/UNIX')
...
def instance_spot_request_new(region='ap-northeast-2', pars=None, refreshfreq=5):
   ...
   req=conn.request_spot_instances(price= pars['max_bid'], image_id=pars['ami'],
                                   instance_type=pars['type'], key_name=keypair,
                                   user_data=pars['user_data'], security_group_ids=[security_id])[0]
...
def monitor_instance_start_fromtask(task_pending, pricelist) :
   ...
   '''
   sum_dur=0;
   for tt in task_pending:  sum_dur+=tt.time_duration
